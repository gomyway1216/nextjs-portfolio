...
ここまでの「損失 → 勾配 → backprop → 山下り」を、全部手で実装するのは大変だ。特にbackprop（微分の連鎖律を全層に適用）を手書きするのは、間違えやすく苦しい。それを**全部自動でやってくれる道具**が **PyTorch（パイトーチ）** だ。Facebook（現Meta）発の、ニューラルネットを作って学習させるためのPythonライブラリで、本編の学習コードもこれで書かれている。
