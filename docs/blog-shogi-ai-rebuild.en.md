...
**Another approach: a saturation guard (a code fix that needs no data).** To be honest here: **sigmoid saturation itself is a mathematical flattening, so it may not vanish completely even with more data.** Bringing decided positions back to 50% stops the region from being "unknown," but the sigmoid's property of going flat at ±2500cp still remains. So in parallel with the data rebuild (the primary path), we're considering a code-only complement that dodges saturation. The idea is simple: **"in any position where the NNUE eval exceeds |cp|>1500, switch just that position to the V3 eval."** Let V3 take over only the region where saturation crushes the distinctions, and leave even positions to the NNUE. It's an insurance play that could stop the collapse without waiting on data. **Which of the two helps, and by how much, is also under verification — results to be appended.**
...
**Effect, and an honest current state.** With the check extension in, **the engine now reads effectively one ply deeper along lines where checks continue.** In the fixed-depth verification the extension does fire — visited node counts tick up (e.g. 3,054 → 3,146 in one position) — while JS/WASM stay identical to the node. That said, to be honest: **whether move 72 itself is fixed, and whether it wins decisively in A/B games, is still under verification** as I write this. A conservative budget-1 extension is a first step at "getting the mating line into the tree"; the power to read deep mates through (which really needs expanding the mate solver's budget, or strengthening a checks-only quiescence search) remains as the next homework. **This section, too, will be updated once the A/B and the move-72 replay results are in.**

**Lessons of this section.** (1) **Don't guess; measure the breakdown.** Because I pinned the culprit of "the endgame is slow" — not pawn-drop-mate, not evaluation, but the quantitative explosion of drop moves — **with numbers**, the countermeasure (extend only checks, not everything uniformly) followed. (2) **Extensions work best in small doses.** "Deeper is stronger" is true, but "more extension means you read deeper" was a lie — the tighter I made the budget (16 → 3 → 1), the better the overall reached depth held up. You can only find the cost/effect knee by measuring. (3) **Speed and strength are still separate axes, but they connect.** On top of the search I made faster in 11.1, this check extension makes the *use* of depth smarter. Speed is the raw material of depth, and depth is the eye that finds the mate.
