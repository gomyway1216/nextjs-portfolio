...
今回追加したのは、その間をつなぐfail-closedなexecution-plan verifier / dispatcherである。役割は次の3つに限定した。
...
6. successorに登録されたproposal、plan、trainをexact bytes / SHA-256で2回読む
7. proposal、plan、train、F / E、3つのtraining contractを相互照合する
8. その後で初めてruntimeと他の入力artifactを確認する
...
`0 < E < 24000`は、proposalとtrainのbytes / SHA-256 / records / parents / games / semantic IDsが完全一致する場合だけ許す。replacementとresamplingは0、emitted orderは保存済みでなければならない。

`E = 0`、つまり24,000件すべてが認証済みforcedの場合は、`STOP-no-trainable-parent-groups`となる。training contract builderへは到達しない。
...
| --- | --- |
| ローカルMac CPU | verifierと150 stdlib tests |
| AWS | 使用なし |
| Firebase / GCP | 使用なし |
| Vercel | 使用なし |
...
code commitは`a0e136296772840ea93066fe8013e4e2ec339a5a`、test commitは`8481a8bc283ca648d23ad11ebd77c0c7e4c0c04a`で、履歴は書き換えていない。

- 新v2 dispatch + route test: 17 / 17 PASS、0.042秒
- repository全stdlib suite: 150 / 150 PASS、10.761秒
- Python compile: PASS
...
敵対ケースには、successor欠落、near / symlink path、wrong schema、v1/v2/WCSC36 hybrid、boolをintとして渡す型alias、F+E不一致、partial / full / all-forced、proposal / train identity drift、replacement、slot drift、contract drift、authority escalation、duplicate key、`NaN`、既存registry driftを含めた。
...
次は、production finalizerが認証したcompletion evidence、実train bytes、parent-accounting proposalを完成させ、それぞれのexact identityを独立検証することである。その後にv2 execution planを作り、別のdata-only PRでready successorを新規追加する。
