...
> **digest authority update（現在の境界）:** production / test teacher runtime factoryが発行したexact frozen facadeを別々のmodule-private `WeakMap`へ登録し、exact 1引数の対応getterだけがreceipt digestを返す。clone、Proxy、plain receipt、別registryのfacadeは拒否し、Proxy trapもsearchも起動しない。plain receipt hashだけではproduction originを証明しない。既存runtime contractと`shogi-floodgate-v7-runtime-receipt-v1\0` domainは変更せず、candidate unionの既存digestと互換である。詳細は[runtime digest authority](./blog-shogi-floodgate-production-runtime-digest-authority.md)を参照する。これはdigest authorityのcode evidenceであり、production coordinator / adapter、checkpoint配線、key、real label、training、weight、live、棋力の証拠ではない。以下の本文はhistorical recordとして残す。
