...
The result first: one real pass over all 36,349 raw receipts fell from 31.706 seconds on the current serial path to 15.680 seconds on the source-closed production twelve-worker path. That is a **2.022x speedup and a 16.026-second saving per pass**. The serial and parallel reports were deep-strictly equal, the candidate manifest had identical canonical bytes, and the stored raw-manifest bytes were unchanged before and after the run.
...
The same completed raw lock and candidate manifest were processed once serially and once through the production twelve-worker path on Node v22.13.0, an Apple M4 Pro with fourteen cores, and 48 GB of RAM. The clock was `process.hrtime.bigint`; process RSS was sampled every 5 ms.
...
The measured speedup is 2.022049x. The observed production peak was about 719 MiB on the 48 GB machine, a modest memory cost for the throughput gained. The 384 MiB × 12 old-generation setting is a V8 configuration ceiling, not reserved memory or measured RSS.

The earlier production-shape emulation reached 2.82x, but 2.02x is the accepted production number. This measurement includes actual candidate revalidation, a held read of the worker bundle, an exact-clean check of the complete tracked Git tree, and a same-revision recheck after worker exit. A faster number that omits the safety boundary is not reported as production speed.
...
The historical 1,088.743-second complete authentication performed four raw passes. Projecting the measured 16.025661-second saving across four passes yields a 64.102646-second saving and a projected 1,024.640 seconds, or about 17.08 minutes.
