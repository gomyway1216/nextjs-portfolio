...
> **Runtime-digest-authority update (current boundary):** The stable and teacher runtimes now have separate production / test authorities that return a registered receipt digest only for the exact facade issued by the matching factory. A plain receipt hash, clone, Proxy, or facade from the other registry carries no origin authority. Existing schemas, digest domains, and the checkpoint v2 format are unchanged. There is still no production coordinator / adapter that passes these authorities into the checkpoint run binding under the same ownership, so the private test-only checkpoint boundary is unchanged. The [runtime digest authority](./blog-shogi-floodgate-production-runtime-digest-authority.en.md) records the details. The body below remains historical and makes no claim about key authority, real labels, training, a weight, live deployment, or playing strength.

> **v2 update (current boundary):** The current checkpoint schema, algorithm, and run binding are v2, and the header HMAC authenticates all of `producer_control`: the parent deadline, abort-drain bound, maximum in-flight count, cancellation policy, and late-settlement policy. After the first terminal failure, it stops new scheduling, aborts each running signal once, calls the controller's `abortAndDrain()` exactly once, and performs a bounded drain within the authenticated time. A v1 stream is rejected before either production capability is called; it can neither resume as v2 nor be re-signed in place, so migration requires a fresh v2 run. This remains a private test-only checkpoint, not evidence about the live environment, teacher labels, training, weights, or playing strength. The body below remains a historical record.
