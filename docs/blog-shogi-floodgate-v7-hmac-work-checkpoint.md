...
> **runtime digest authority update（現在の境界）:** stable / teacher runtimeに、factoryが発行したexact facadeだけから登録済みreceipt digestを返すproduction / test別authorityが追加された。plain receipt hash、clone、Proxy、別registryのfacadeはorigin authorityにならない。既存schema / digest domain / checkpoint v2 formatは変更していない。ただし、このauthorityをcheckpoint run bindingへ同じownership内で渡すproduction coordinator / adapterはまだないため、private test-only checkpointという境界は変わらない。詳細は[runtime digest authority](./blog-shogi-floodgate-production-runtime-digest-authority.md)に記録した。以下の本文はhistorical recordとして残し、key authority、real label、training、weight、live、棋力を主張しない。

> **v2 update（現在の境界）:** 現行checkpoint schema / algorithm / run bindingはv2で、header HMACは`producer_control`全体（parent deadline、abort-drain bound、最大in-flight数、cancel policy、late-settlement policy）を認証する。最初のterminal failure後は新規scheduleを止め、running signalを各1回abortし、controllerの`abortAndDrain()`を1回だけ呼んで認証済み時間内にbounded drainする。v1 streamはproduction capabilityを呼ぶ前に拒否され、v2としてresumeもin-place re-signもできないため、移行にはfresh v2 runが必要である。この境界はprivate test-only checkpointで、live環境、teacher label、training、weight、棋力の証拠ではない。以下の本文はhistorical recordとして残す。
...
review follow-upでは、`requiredInteger`がparser-validな`-0`を通し、後段のcanonical hashingまで負数ゼロを残す境界が見つかった。capture時に`+0`へ正規化し、同じhelperを直接通す回帰testを追加した。最初のCIで唯一落ちたのは今回の差分外だったreusable-pool testで、新規v7 testはすべて通っていた。失敗stackのreceiverはparentではなく`Array.value`で、Vitest realm全体へ一時的に追加した`Object.prototype.then`が無関係なPromise解決を拾っていた。同一SHAのfailed job再実行は成功したが、再発防止のため検査を既存のbundled child-process fixtureへ移した。隔離caseはNode v22.13.0で10/10反復成功した。これはruntimeやweightの変更ではなく、test isolationの修正である。

このPRのtest inputはsynthetic parent / result / keyだけである。real Floodgate training row、fresh selection、fresh / legacy final holdoutを開かず、production engineを起動しない。selection / final labelのpathやreaderをcheckpoint APIへ渡さない。review follow-up後のNode v22.13.0ではcandidate union 34本、completed-parent 12本、checkpoint 18本のfocused Vitestが64/64、proposerを含む4 files / 117 tests、全111 files / 1,904 testsが通った。TypeScript、Python ML stdlib 58/58、Next production build 193/193 pages、scoped ESLint `--max-warnings=0`、Prettierも成功した。repository全体のESLintは0 errors / 157 existing warningsだった。これらはsemantic capture、compact-evidence re-verification、HMAC resume、wrong binding、forced skip、key isolation、crash境界を検査する。
