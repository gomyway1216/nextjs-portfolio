...
> **v2 update (current boundary):** The current scanner verifies the v2 checkpoint identity and authenticated `producer_control`. The parent deadline, one abort per running signal, one controller abort-drain call, drain timeout, and timer-cancellation handling are implemented and verified at the test-only v2 boundary, but are not wired into the production coordinator or live teacher. Statements below that a valid 24k run was not complete and that timeout / cancellation was the next stage, together with the old measured values, are a historical v1 record and must not be treated as current v2 evidence. This is not evidence of real labels, training, weights, matches, or playing strength.
