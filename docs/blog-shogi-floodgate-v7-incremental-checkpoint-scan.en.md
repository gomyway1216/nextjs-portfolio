...
At scan start, the scanner therefore snapshots the held file's `dev`, `ino`, type, mode, owner, link count, size, `mtime`, and `ctime`, then requires the same descriptor to match the full snapshot after scanning. After the final sealed scan, it uses `lstat` on the stage pathname and checks, before success, that the entry set contains only `work.jsonl`, that the pathname snapshot matches the held file, and that the held descriptor still matches the same snapshot. Open and reopen operations do not follow symlinks.
