...
This candidate was published as ready-for-review PR #472. On the post-review implementation head `da7e9dd`, all 6 reported GitHub checks passed, including 2 / 2 required status checks; all 5 actionable review threads were addressed and zero remained unresolved. PR #472 was subsequently integrated with regular merge commit `6e5197fb9a9200cc1b00db1ee34e072b9de84ea2`. The open and pending language and the historical measurements retained in the [machine-readable evidence](./data/floodgate-v7-prefix-100-same-lock-one-shot-2026-07-16.json) preserve its evidence-refresh snapshot. They are neither rewritten nor reinterpreted as results from the current unmerged follow-up candidate.
...
Independent registry audit confirmed `e8a9197608cb48b1160b6707d97b0c4f78f90a1d` as an evidence-backed viable candidate. `0f3cadb` is its ancestor, it contains the required artifacts, and both the accepted and confirmation production full-verifier runs exited zero from clean detached worktrees.

A separate follow-up candidate is now published as ready-for-review [PR #474](https://github.com/gomyway1216/nextjs-portfolio/pull/474), but it remains unmerged. PR #474 implements the `e8a9197` binding and the fail-before-install `source-tree-and-pinned-receipt-evidence Git closure` required by provisioner and preflight v2. It derives the repository root from the fixed current-EUID user-info home and, under standard Git ignore rules, requires a clean nonignored worktree at the exact revision. It runs the source-tree check twice, before and after artifact inspection, comparing every tracked file's bytes and mode with HEAD each time. It also reads seven pinned receipt and evidence artifacts directly from the worktree and checks their pinned Git blobs, receipt content, and producer ancestry. This is therefore not a metadata-only check. Ignored entries are outside the closure; external role-bundle outputs are not read, and the full role-bundle verifier does not run. Its readiness receipt discloses no path, revision, digest, or private identity, while a separate private single-use claim binds that receipt to the EUID and home. The provisioner checks the closure and identity binding before current-key binding, enrollment, entropy acquisition, or create-only installation. Prefix-100 preflight rechecks the same closure and identity after claiming the registry's fixed configuration and before namespace or deployment-key checks. Either failure starts neither installation nor a gate. Readiness success does not issue registry or gate authority.

The readiness leaf is the newly introduced `shogi-floodgate-v7-production-connector-verifier-readiness-v1`. Existing boundaries that gain a closure-confirmation field or the `verifier-readiness` failure phase are not treated as compatible v1 receipts: provisioner success, provisioning-CLI failure, the public preflight core and claim boundary, the under-lock preflight outcome, and preflight-CLI success and failure each advance to v2. This prevents an old consumer from accepting a receipt that lacks the closure field as a new success.

PR #474 is an unmerged candidate, not reviewed production authority. It has performed zero production operations. It has added no registry provisioning, kill drill, prefix-100 / 500 / final-24000 invocation, teacher generation, training, weight, match, or live activation. Neither the production registry nor a real gate will be operated before the authenticated training-label finalizer is regular-merged.
...
The current safe order is therefore:

1. The same-lock one-shot completed final validation and review and is regular-merged.
2. Review PR #474's `e8a9197` binding, source-tree and pinned-evidence closure, private identity binding, and fail-before-install / preflight recheck, then regular-merge it.
