...
> **Runtime-digest-authority update (current boundary):** The stable and teacher production factories now register each exact facade in a module-private registry, adding an origin check that a SHA-256 computed from a plain receipt cannot replace. Production and test registries are separate, and a clone, Proxy, plain object, cross-registry facade, or wrong arity fails closed. Schemas and digest domains are unchanged. Attempt 6 nevertheless remains test-only synthetic scanner evidence; the new authority does not retroactively make it an official receipt or production origin. The production coordinator / all-settled owning adapter and checkpoint wiring are also absent. See the [runtime digest authority](./blog-shogi-floodgate-production-runtime-digest-authority.en.md). The body and table below remain the historical record from before this authority was added and make no claim about a key, real labels, training, a weight, A/B, live deployment, or playing strength.
