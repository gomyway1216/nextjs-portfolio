...
On the first post-remediation delivery head, `6d00f65d6bb1772b336021a8ac425437fd7f9727`, Security, E2E, and Vercel passed, while the Darwin job on a macOS 26.4 runner failed one of 22 native-launcher tests. The test assumed that a nonexistent `DYLD_INSERT_LIBRARIES` value would always be stripped by protected `caffeinate` and the child would exit 0. On the newer runner, the loader instead rejected startup with status 6 before the attested child began. Both outcomes prevent injection state from reaching the attested child, so this was a cross-version test assumption rather than a production-implementation failure. Commit `494ad72e55a4eaa890b775bbeafc9a52d9e2bf4d` now treats either “stripped and successfully attested” or “rejected by the protected loader with status 6 before child startup” as safe; the Node 22 targeted test returned to 22 / 22. CI for `6d00f65...` is not a final merge gate, and every check is rerun on the later head.
