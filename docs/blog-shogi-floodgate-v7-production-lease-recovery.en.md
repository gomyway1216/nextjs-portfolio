...
After these fixes, the focused outer, runner, checkpoint-connector, and CLI set passed 187 / 187 on the required Node version. A separate independent rerun passed the two stage-authorization/publication files at 147 / 147 in 8.88 seconds of wall time. Neither focused result replaces the final combined rerun.

The first combined rerun used the wrong Node version, so the CLI's fixed runtime guard correctly stopped before runner load and the invalid attempt ended at 271 / 280. That was not an implementation regression. After switching to the required Node version, the next rerun found two source-structure allowlists while implementation was still moving and ended at 278 / 280. On the final stable tree, all 337 tests across seven files passed in 9.61 seconds of Vitest time and 10.06 seconds of wall time with zero swaps. TypeScript, scoped ESLint, the Prettier check, and `git diff --check` also passed. The [machine-readable evidence](./data/floodgate-v7-production-lease-recovery-2026-07-15.json) records the details. The intermediate attempts remain visible without misclassifying a wrong-runtime failure as an implementation failure.
...
The additional full regression run passed all 2,590 tests across 138 files in 152.87 seconds of Vitest time and 153.53 seconds of wall time, with a 4,038,459,392-byte maximum resident set and zero swaps. That full suite still includes no real production gate, real stale recovery, training, or weight activation.

Every successful test is a local fixture or local child process. No test used a real stale production source, real reboot, or real production gate.
