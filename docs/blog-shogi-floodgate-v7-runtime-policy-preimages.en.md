...
The remote head then advanced to `64ffac635ff826ae22368bbdc0404452623f4e14`, and CI run `29645550110` reached terminal **COMPLETED / FAILURE**. Its only failure was the `Verify external trust-root public API surface` step in External trust-root job `88083075424`. `Test and build` completed **SUCCESS**, including lint, unit tests, deadline calibration, ML contracts, and the production build; Darwin, E2E, Security, and Vercel also all completed **SUCCESS**. The retained symbol-graph artifact has ID `8429932370`, digest `sha256:5fe40bb4cfcc5a5d967dbd001b0f30878dc23015b89d3a1b1635bae31e636baa`, and exact name `floodgate-v7-public-symbol-graphs-macOS-ARM64-6c871485b3d65726adad72fad77aafe94e976590-1`. The `6c871485b3d65726adad72fad77aafe94e976590` in that name is GitHub's temporary PR merge commit that overlays head `64ffac635ff826ae22368bbdc0404452623f4e14` onto main `0601268a57af32c910b785c3f79da647d3fbb428` for CI; it is not an artifact from a stale head.

The artifact's exact metadata is Xcode 26.5, generator `Apple Swift version 6.3.2 (swiftlang-6.3.2.1.108 clang-2100.1.1.101)`, format 0.6.0, and arm64 macOS with minimum version 13. Its public/SPI surface after synthesized members are skipped is 491 symbols, 579 relationships, and SHA-256 `539e6c39aabf364b464b05b00517c18da061e23987aceb54c8fcbf0825991123`. Against local Xcode 15.3 / Swift 5.10 at 491 symbols, 542 relationships, and SHA-256 `3e040bc6097a0d7ab1ea7c511b0e6fd32c8a2d7a5c5076ee00beba1a21ae8160`, the exact delta is zero symbol additions, zero symbol removals, 37 relationship additions, and zero relationship removals. All 37 additions are `conformsTo Swift.SendableMetatype`. Only five existing precise identifiers have declaration-rendering changes, each reflecting an `@Sendable` annotation already present in source rather than a source public-API addition or removal.

Local technical commit/tree `f5a264d4d062a4e30e6b8c6de3c104acf432996d` / `914b2bf91ca722c89b82714efdd22b835affefb9` therefore allows only the exact Xcode 15.3 / Swift 5.10 and Xcode 26.5 / Swift 6.3.2 profiles. Independent review found one P2 in that commit: its behavioral self-check omitted unknown format, platform, and module contexts plus mixed base-and-shard profiles. The record retains that CHANGES_REQUESTED result rather than rewriting it as a pass. Follow-up commit/tree `7d9281129e0d6447a70c1cbbbf20d3ec967d940b` / `1355122d15bb9284cbb79713c81a5287931879a8` adds executable rejection checks for five contexts: unknown generator, format, platform, or module, and mixed base/shard profiles. Local validation passes, and two independent exact-commit reviews found P0 / P1 / P2 = 0 / 0 / 0 at `7d9281129e0d6447a70c1cbbbf20d3ec967d940b`; the local technical review gate therefore passes while unknown or mixed profiles fail closed. Both commits remain local and unpushed after the remote head, and the latest-head CI rerun is `NOT_STARTED`. The current four publication paths themselves have not yet passed review or CI. This remains a structural gate over direct signatures, `Self`, extension, and conformance surfaces; it does not prove the bodies of existing public symbols or arbitrary wrapper, byte-decoder, generic, or dynamic-cast behavior, so source security review and adversarial tests remain required.

The working tree also resolves Git without a shell through only `/usr/bin:/bin`, scopes the CI evidence to the next arbitrary job boundary, and retries once after `SIGABRT` or `SIGKILL` when stdout is empty while logging only a sanitized outcome shape with no raw child output. Empty stderr is not a retry predicate. Failure diagnostics record only whether stderr is empty and its UTF-8 byte count, never its raw contents; only the `status === 0` branch asserts empty stderr. The first signal is never accepted as success: the retry must still end with status 0 or 6 and a null signal. Independent local review and validation found five further P1 and thirty-six further P2 issues during this remediation. The technical fixes were preserved as intermediate commit `eba6e9ecbd271fa4d8354fe1552a8123ac326959` and follow-up commit `735398093f7c839c8c2a97f33ef96607961bd829`; exact review of final commit/tree `735398093f7c839c8c2a97f33ef96607961bd829` / `5f8b873ffe1d15d5a9efc50e7e986478d826f3bc` passed with P0 / P1 / P2 = 0 / 0 / 0. Publication-hardening commit/tree `f231f30a3a354ce1895553a90a775b85691376e6` / `259dceb28f96b90194ef03808f88d6a59effd339` also passed one exact review before a separate audit found one P2: only the “empty-output” record was broader than the real predicate. That finding remains in history while the label is corrected to “empty-stdout.” Exact review of final local evidence commit/tree `69e982e52966d24f21c994cf42494d4234e4420e` / `192c847d4eb0396654b886acb7e930dea493fbdf` across the four paths also passed with P0 / P1 / P2 = 0 / 0 / 0. The previous implementation-security-review PASS and intermediate CHANGES_REQUESTED results also remain in history. The current publication working-diff audit found one P2 because its snapshot timestamp predated the second exact review of `7d928112`; the record keeps the finding and fixes it. At the recorded `2026-07-18T07:04:05-07:00` snapshot, the remote head is `64ffac635ff826ae22368bbdc0404452623f4e14`; both subsequent toolchain-calibration commits remain local and unpushed, and their latest-head rerun is `NOT_STARTED`. Two outdated evidence-test review threads have been resolved with evidence. The symbol-graph thread remains unresolved until External is green on the latest head. This in-progress four-path publication update has not itself undergone exact-commit review or CI, so the technical commits' PASS is not transferred to the current publication content. PR validation remains **IN_PROGRESS / STOP**.

Swift source tests are **58 / 58 PASS** across every-byte mutation of fixed argv, cwd, and environment; runtime-install framing, fixed-policy, and path-metadata drift; five policy-digest substitutions; install swapping; Node-identity drift; manifest drift; and closure-drift rejection at all four public handoff entrypoints. The runtime-install record's nine variable digest fields are constrained only to be nonzero and pairwise distinct during canonical decoding; six of them carry runtime identities. This article claims neither that every runtime-install byte is fixed nor that the identities and policies have been observed in a real environment. The Swift cross-parser subset is 4 / 4 and the independent Node parser is **6 / 6 PASS**. The public symbol graph confirms exactly four composed entrypoints and six non-public partial entrypoints. The full TypeScript no-emit check, targeted ESLint, targeted Prettier, Python symbol-graph-checker compilation, and `git diff --check` also pass. Before the failed attempts, `npm test -- --maxWorkers=6` ran from 02:44:01 through 02:49:30 and passed 181 / 181 files with 3,232 passes, one skip, and zero failures in 329.30 seconds of Vitest time and 329.60 seconds of wall-clock time. HEAD was `3adfd0651e22ecb801b958eef8c9ca00f054a52e`, but the working tree contained uncommitted publication content, so this run is not the final-revision gate. A later high-contention attempt overlapped the build, Swift tests, and active evidence edits; it passed 179 of 181 files and recorded 3,230 passes, one skip, and two failures out of 3,233 tests in 341.08 seconds, so it is non-authoritative. Its five-second sibling-teacher timeout then passed 12 / 12 in isolation in 14.93 seconds. A subsequent default-worker Node full-suite rerun passed 180 of 181 files and recorded 3,231 passes, one skip, and one startup timeout; the failed stable-WASM file then passed 61 / 61 in isolation. This is consistent with local resource scheduling but does not establish the cause or a PR #501 product regression. The final run bounded scheduling to four workers and passed **181 / 181 files with 3,232 passes, one skip, and zero failures** at `f231f30a3a354ce1895553a90a775b85691376e6` in 314.19 seconds. All attempts remain in the data rather than rewriting intermediate failures as green; this PR does not change production timeouts, live weights, or configuration.
