...
その後、remote headは`64ffac635ff826ae22368bbdc0404452623f4e14`まで進み、CI run `29645550110`はterminal **COMPLETED / FAILURE**となった。失敗はExternal trust-root job `88083075424`の`Verify external trust-root public API surface` stepだけである。`Test and build`はlint、unit tests、deadline calibration、ML contract、production buildを含め**SUCCESS**し、Darwin、E2E、Security、Vercelもすべて**SUCCESS**した。artifact ID `8429932370`、digest `sha256:5fe40bb4cfcc5a5d967dbd001b0f30878dc23015b89d3a1b1635bae31e636baa`、exact name `floodgate-v7-public-symbol-graphs-macOS-ARM64-6c871485b3d65726adad72fad77aafe94e976590-1`を取得した。name中の`6c871485b3d65726adad72fad77aafe94e976590`は、GitHubがhead `64ffac635ff826ae22368bbdc0404452623f4e14`をmain `0601268a57af32c910b785c3f79da647d3fbb428`へ重ねてCI用に作ったtemporary PR merge commitであり、古いheadのartifactではない。

artifactのexact metadataはXcode 26.5、generator `Apple Swift version 6.3.2 (swiftlang-6.3.2.1.108 clang-2100.1.1.101)`、format 0.6.0、arm64 macOS minimum 13だった。合成memberを除いたpublic / SPI surfaceは491 symbol、579 relationship、SHA-256 `539e6c39aabf364b464b05b00517c18da061e23987aceb54c8fcbf0825991123`である。local Xcode 15.3 / Swift 5.10の491 symbol、542 relationship、SHA-256 `3e040bc6097a0d7ab1ea7c511b0e6fd32c8a2d7a5c5076ee00beba1a21ae8160`との差は、symbol追加0 / 削除0、relationship追加37 / 削除0だった。追加37件はすべて`conformsTo Swift.SendableMetatype`で、宣言表示が変わったのは既存のprecise ID 5件だけ、いずれもsourceに既にある`@Sendable`のtoolchain別renderingであり、source public APIの追加・削除ではない。

そこでlocal technical commit / tree `f5a264d4d062a4e30e6b8c6de3c104acf432996d` / `914b2bf91ca722c89b82714efdd22b835affefb9`は、Xcode 15.3 / Swift 5.10とXcode 26.5 / Swift 6.3.2のexact 2 profileだけを許可した。独立reviewはこのcommitに、unknown format / platform / moduleとmixed base + shard profileをbehavioral self-checkしていないP2を1件見つけた。履歴をPASSへ書き換えず、follow-up commit / tree `7d9281129e0d6447a70c1cbbbf20d3ec967d940b` / `1355122d15bb9284cbb79713c81a5287931879a8`でunknown generator / format / platform / moduleとmixed base / shardの5 contextを実行してすべて拒否するself-checkを追加した。local validationはPASSし、さらに2名の独立exact-commit reviewが`7d9281129e0d6447a70c1cbbbf20d3ec967d940b`をP0 / P1 / P2 = 0 / 0 / 0で確認したため、未知または混在したprofileはfail closedする技術修正のlocal review gateはPASSである。ただし両commitはremote head後のlocal unpushed commitで、latest-head CI rerunは`NOT_STARTED`である。現在のpublication 4 path自身もまだreviewもCIも受けていない。direct signature / `Self` / extension / conformance surfaceの構造gateであり、既存public symbolのbody、arbitrary wrapper / byte decoder / generic / dynamic castの挙動までは証明しないため、source security reviewとadversarial testは引き続き必須である。

shellを使わない`/usr/bin:/bin`限定のGit解決、次の任意job境界までのCI scope、`SIGABRT` / `SIGKILL`時にstdoutが空なら一度だけ再試行してraw child outputを含まないsanitized outcome shapeだけを記録する診断も追加した。stderrが空であることは再試行条件ではない。failure diagnosticには空否とUTF-8 byte数だけを記録してraw内容は出さず、`status === 0`のbranchだけstderr emptyをassertする。最初のsignal自体は成功扱いせず、再試行後も最終`status` 0または6と`signal === null`を必須にする。独立local reviewとlocal validationはこの修正過程で追加P1を5件、P2を36件見つけた。技術修正は中間commit `eba6e9ecbd271fa4d8354fe1552a8123ac326959`と追補commit `735398093f7c839c8c2a97f33ef96607961bd829`へ分け、最終commit / tree `735398093f7c839c8c2a97f33ef96607961bd829` / `5f8b873ffe1d15d5a9efc50e7e986478d826f3bc`のexact reviewはP0 / P1 / P2 = 0 / 0 / 0でPASSした。publication hardening commit / tree `f231f30a3a354ce1895553a90a775b85691376e6` / `259dceb28f96b90194ef03808f88d6a59effd339`も一度exact PASSした後、別監査が「empty-output」という記録だけが実predicateより広いP2を発見し、発見履歴を保持したまま「empty-stdout」へ訂正した。final local evidence commit / tree `69e982e52966d24f21c994cf42494d4234e4420e` / `192c847d4eb0396654b886acb7e930dea493fbdf`の4 path exact reviewもP0 / P1 / P2 = 0 / 0 / 0でPASSした。以前の実装security review PASSと途中のCHANGES_REQUESTEDも履歴として残す。今回のpublication working-diff監査は、2件目の`7d928112` exact reviewより前のsnapshot時刻を使ったP2を1件見つけ、履歴を残して修正した。記録時刻`2026-07-18T07:04:05-07:00`ではremote headは`64ffac635ff826ae22368bbdc0404452623f4e14`、その後のtoolchain校正2 commitはlocal unpushed、latest-head rerunは`NOT_STARTED`である。3つのremote review threadのうちoutdatedになったevidence test 2件は根拠付きでresolved、symbol-graph thread 1件はlatest-head External greenが得られるまでunresolvedのままにする。この更新中のpublication 4 pathはexact-commit reviewもCIも未実施なので、技術commitのPASSを現在のpublication内容へ移し替えない。PR validationは**IN_PROGRESS / STOP**のままである。

fixed argv / cwd / environmentの全byte mutation、runtime-install framing / 固定policy / path metadata drift、5種類のpolicy digest substitution、install swap、Node identity drift、manifest drift、4つのpublic handoff入口のclosure drift拒否を含むSwift source testは**58 / 58 PASS**した。runtime install recordの9つの可変digest fieldはcanonical decode時にnonzero / pairwise-distinctだけを要求し、そのうち6つがruntime identityを保持する。全byteの固定や、実環境でのidentity / policy観測完了は主張しない。このうちSwift cross-parserは4 / 4、独立Node parserは**6 / 6 PASS**である。public symbol graphはcomposed入口4、非公開partial入口6をexactに確認した。TypeScript全体型検査、対象ESLint、対象Prettier、Python symbol-graph checker compile、`git diff --check`もPASSした。これより前の`npm test -- --maxWorkers=6`は02:44:01から02:49:30まで走り、181 / 181 files、3232 PASS / 1 skip / 0 FAIL、Vitest 329.30秒 / wall clock 329.60秒でPASSした。HEADは`3adfd0651e22ecb801b958eef8c9ca00f054a52e`だったがworking treeはuncommittedのpublication contentを含むため、最終revision gateには使わない。さらに後の高負荷並行attemptはbuild / Swift test / evidence編集と重なり、181 file中179 PASS、3233 test中3230 PASS / 1 skip / 2 FAIL、341.08秒だったためnon-authoritativeとして破棄した。そのうちsibling teacherの5秒timeoutは単独再検査で12 / 12 PASS、14.93秒だった。続くNode full suiteのdefault worker再実行は181 file中180 PASS、3233 test中3231 PASS / 1 skip / 1 startup-timeoutで、失敗したstable-WASM fileは単独再検査で61 / 61 PASSした。これはlocal resource schedulingとの整合性はあるが原因を確定したものではなく、PR #501のproduct regressionも確立していない。並列度を4 workerに固定した最終実行は`f231f30a3a354ce1895553a90a775b85691376e6`で**181 / 181 files、3232 PASS / 1 skip / 0 FAIL**、314.19秒だった。途中の失敗を最終PASSへ書き換えず全attemptをdataへ残し、このPRでproduction timeout、live weights、設定は変更しない。
