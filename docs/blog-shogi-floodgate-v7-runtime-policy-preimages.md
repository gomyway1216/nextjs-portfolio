...
日英記事、data、evidence test、履歴照合用CI設定は別publication reviewで確認する。初回はchronology / scope / gateのP1を3件と、mutation / 日英文言 / false-passのP2を4件発見した。独立provenance監査は9欄の分類、ordered gate、counter provenanceのP2を3件、green snapshot監査はschema key、再度のchronology、意図的red testのP2をさらに3件発見した。修正済み合計はP1 3件 / P2 10件で、3回のCHANGES_REQUESTED履歴をdataに残す。CI差分はfull-history checkoutと25分timeoutによるprovenance gateのためだけで、実装tree security reviewの対象には含めない。発見時の記録をPASSへ書き換えず、各修正revisionとreview scopeを機械可読記録に分離する。
