...
The 1.5-second, color-swapped 56-game screen finished **31 candidate wins, 20 production wins, and five draws: 67/112 half-points, or 59.8%**. The pass line was 62. Every move was legal, openings were unique, and there were no technical faults. This is the first short-screen pass, not yet proof that the model is reliably stronger. A separate 96-game run with unused seeds and openings is now testing whether the one-sided 95% paired-bootstrap lower bound is strictly above 50%. Production weights remain unchanged.

In parallel, three HalfKP candidates warm-started from the near-pass checkpoint and trained for eight epochs on 278,736 legal sibling rows. They combined pairwise/listwise move ranking with a one-to-one replay of 500,000 ordinary value positions. Value MAE improved by 121–161cp, but independent pair accuracy fell by 0.53–0.94 points and top-1 by 1.19–1.81 points. No checkpoint was admitted, so this family stopped before matches. For the next cycle, we built an eight-shard generator that mines only positions where production browser search and the depth-12 teacher disagree, then independently scores **every legal child position**. A three-parent smoke covered 239/239 legal moves, child SFENs, and perspective signs, with byte-identical reruns.

The change that first crossed a short playing gate was neither a larger model nor more data. **It retained half of production's practical ordering and averaged away an over-large training update.** The unused 96-game confirmation and then 768-game formal test decide whether that effect reproduces. A failure at either stage still means no deployment.
