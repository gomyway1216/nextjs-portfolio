...
async function main(): Promise<void> {
  if (process.argv.length !== 2) {
    throw new Error(
      "this read-only v1r11 plan authenticator accepts no arguments",
    );
  }
  const authenticated = await authenticateHalfkp81Depth18TeacherPlan(
    HALFKP81_DEPTH18_YANEURA_ONLY_V1R11_DEFAULT_PLAN_PATH,
  );
  const preregistration = authenticated.plan.preregistration as Record<
    string,
    unknown
  >;
  if (
    ...
    preregistration.bytes !==
      HALFKP81_DEPTH18_YANEURA_ONLY_V1R11_PREREGISTRATION_IDENTITY.bytes ||
    preregistration.sha256 !==
      HALFKP81_DEPTH18_YANEURA_ONLY_V1R11_PREREGISTRATION_IDENTITY.sha256
  ) {
    throw new Error("authenticated plan is not Yaneura-only v1r11");
  }
  process.stdout.write(
    `${JSON.stringify({
      ...
      power_continuity_required: true,
      authority: {
...
main().catch((error: unknown) => {
  process.stderr.write(
    `[halfkp81-depth18-yaneura-only-v1r11-plan-authenticate] STOP: ${
