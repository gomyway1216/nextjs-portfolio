...
async function main(): Promise<void> {
  if (process.argv.length !== 2) {
    throw new Error(
      `this read-only v1r9 plan authenticator accepts no arguments and reads ${HALFKP81_DEPTH18_YANEURA_ONLY_V1R9_DEFAULT_PLAN_PATH}`,
    );
  }
  ...
  if (
    ...
    throw new Error("authenticated plan is not Yaneura-only v1r9");
  }
...
main().catch((error: unknown) => {
  process.stderr.write(
    `[halfkp81-depth18-yaneura-only-v1r9-plan-authenticate] STOP: ${
