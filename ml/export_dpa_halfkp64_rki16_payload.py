#!/usr/bin/env python3
"""Export a DPA-HalfKP64-RKI16 checkpoint to the fixed integer runtime payload.

The scales and byte layout are imported from the runtime reference.  They are
not searched or fitted.  Board/hand and relative-factor weights use signed
int16 Q127, the 64 reserved first-bias int32 values are always zero, and both
output vectors use signed int16 Q64.  Non-finite or clipped coordinates abort.
"""
...
from dpa_halfkp64_rki16_nnue import (
    DpaHalfkp64Rki16NNUE,
    HIDDEN,
    PAD_INDEX,
    RELATIVE_HIDDEN,
)
from dpa_halfkp64_rki16_runtime_int_reference import (
    ACTIVATION_SCALE,
...
from train_dpa_halfkp64_rki16_nnue import CHECKPOINT_FAMILY
...
PAYLOAD_FORMAT = "dpa-halfkp64-rki16-runtime-v1"
...
class QuantizedDpaHalfkp64Rki16:
...
def quantize_model(
...
) -> tuple[QuantizedDpaHalfkp64Rki16, dict[str, object]]:
    ...
    if tuple(tensors.relative_self.shape) != (KING_BUCKETS, RELATIVE_HIDDEN):
        raise DpaPayloadExportError("relative self shape differs from runtime layout")
    if tuple(tensors.relative_other.shape) != (KING_BUCKETS, RELATIVE_HIDDEN):
        raise DpaPayloadExportError("relative other shape differs from runtime layout")
    if tuple(tensors.relative_output.shape) != (RELATIVE_HIDDEN,):
        raise DpaPayloadExportError("relative output shape differs from runtime layout")
    ...
    relative_self, relative_self_stats = _quantize_i16(
        tensors.relative_self,
        scale=ACTIVATION_SCALE,
        name="relative_self",
    )
    relative_other, relative_other_stats = _quantize_i16(
        tensors.relative_other,
        scale=ACTIVATION_SCALE,
        name="relative_other",
    )
    relative_output, relative_output_stats = _quantize_i16(
        tensors.relative_output,
        scale=OUTPUT_WEIGHT_SCALE,
    ...
    quantization: dict[str, object] = {
        ...
        "relative_self": relative_self_stats,
        "relative_other": relative_other_stats,
        "relative_output": relative_output_stats,
    }
    quantized_stats = (
        board_stats,
        hand_stats,
        output_stats,
        relative_self_stats,
        relative_other_stats,
        relative_output_stats,
    )
    clipping = sum(
        int(stats["clipping_coordinates"])
        for stats in quantized_stats
    )
    nonfinite = sum(
    ...
    assert relative_self is not None and relative_other is not None
    assert relative_output is not None
    return (
        QuantizedDpaHalfkp64Rki16(
            ...
            relative_self=relative_self.contiguous(),
            relative_other=relative_other.contiguous(),
            relative_output=relative_output.contiguous(),
        ),
        quantization,
    )


def _validated_payload(q: QuantizedDpaHalfkp64Rki16) -> None:
    ...
    if q.relative_self.dtype != torch.int16 or tuple(q.relative_self.shape) != (
        KING_BUCKETS,
        RELATIVE_HIDDEN,
    ):
        raise DpaPayloadExportError("quantized relative self is malformed")
    if q.relative_other.dtype != torch.int16 or tuple(q.relative_other.shape) != (
        KING_BUCKETS,
        RELATIVE_HIDDEN,
    ):
        raise DpaPayloadExportError("quantized relative other is malformed")
    if q.relative_output.dtype != torch.int16 or tuple(q.relative_output.shape) != (
        RELATIVE_HIDDEN,
    ):
        raise DpaPayloadExportError("quantized relative output is malformed")


@torch.no_grad()
def integer_forward(
    q: QuantizedDpaHalfkp64Rki16,
    ...
    other_buckets = (KING_BUCKETS - 1) - bucket.flip(1)
    relative_features = torch.bitwise_right_shift(
        q.relative_self[bucket].long()
        * q.relative_other[other_buckets].long(),
        7,
    )
    relative_output = (
        (relative_features[:, 0] - relative_features[:, 1])
        * q.relative_output.long()
    ).sum(dim=1)
    raw_output = main_output + relative_output
...
def float_integer_parity_report(
    ...
    theoretical_output_abs_max = (
        HIDDEN * INT16_MAX * ACTIVATION_SCALE
        + RELATIVE_HIDDEN * INT16_MAX * (ACTIVATION_SCALE * 2)
    )
...
def _write_payload(path: Path, q: QuantizedDpaHalfkp64Rki16) -> None:
    with path.open("xb") as target:
        ...
        _write_integer_tensor(target, q.relative_self, "h")
        _write_integer_tensor(target, q.relative_other, "h")
        _write_integer_tensor(target, q.relative_output, "h")
        target.flush()
...
def _load_model(checkpoint: Path) -> DpaHalfkp64Rki16NNUE:
    ...
    try:
        ...
        report: dict[str, object] = {
            ...
            "layout": {
                ...
                "relative_self": LAYOUT.relative_self,
                "relative_other": LAYOUT.relative_other,
                "relative_output": LAYOUT.relative_output,
...
def parity_batches_from_jsonl(
    ...
    *,
    ...
    from train_dpa_halfkp64_rki16_nnue import TrainingExample, collate_examples
