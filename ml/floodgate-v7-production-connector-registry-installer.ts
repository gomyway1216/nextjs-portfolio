 ...
 * production-connector registry record. The final name is committed with link(2),
...
import {
  ...
  type FloodgateV7ProductionConnectorRegistryExecutionBoundary,
...
export const FLOODGATE_V7_PRODUCTION_CONNECTOR_REGISTRY_INSTALLER_CLAIM_BOUNDARY =
  "persists-one-exact-canonical-production-connector-registry-without-adopting-overwriting-rotating-or-disclosing-registry-contents-path-identities-authority-runtime-training-live-or-strength-claims" as const;
export const FLOODGATE_V7_PRODUCTION_CONNECTOR_REGISTRY_INSTALLER_TRUST_BOUNDARY =
  "trusted-current-euid-userinfo-home-local-posix-filesystem-production-connector-registry-serializer-and-current-js-realm-intrinsics-v1" as const;
export const FLOODGATE_V7_PRODUCTION_CONNECTOR_REGISTRY_INSTALLER_ALGORITHM =
  "canonical-registry-staged-fsync-readback-hard-link-no-clobber-directory-fsync-staging-unlink-directory-fsync-reopen-revalidation-v1" as const;

export type FloodgateV7ProductionConnectorRegistryInstallerExecutionBoundary =
  | "production-fixed-current-euid-userinfo-home-production-connector-registry-installation"
  | "test-only-injected-current-euid-home-production-connector-registry-installation";

export type FloodgateV7ProductionConnectorRegistryInstallerFailpoint =
...
export type FloodgateV7ProductionConnectorRegistryInstallerPhase =
  | "capture"
  | "production-identity"
  | "registry-validation"
  | "namespace"
...
export type FloodgateV7ProductionConnectorRegistryInstallerDurability =
  | "no-installation-change-established"
  | "managed-prefix-may-exist-registry-absent"
  | "managed-prefix-may-exist-existing-registry-not-adopted"
...
export type FloodgateV7ProductionConnectorRegistryInstallerRetryDisposition =
  | "safe-to-retry-after-not-installed"
  | "do-not-retry-existing-registry"
  | "manual-reconciliation-required";

export interface FloodgateV7ProductionConnectorRegistryInstallerReceipt<
  TBoundary extends
    FloodgateV7ProductionConnectorRegistryInstallerExecutionBoundary =
    FloodgateV7ProductionConnectorRegistryInstallerExecutionBoundary,
> {
  readonly contract: typeof FLOODGATE_V7_PRODUCTION_CONNECTOR_REGISTRY_INSTALLER_CONTRACT;
  readonly status: typeof FLOODGATE_V7_PRODUCTION_CONNECTOR_REGISTRY_INSTALLER_STATUS;
  readonly claim_boundary: typeof FLOODGATE_V7_PRODUCTION_CONNECTOR_REGISTRY_INSTALLER_CLAIM_BOUNDARY;
  readonly trust_boundary: typeof FLOODGATE_V7_PRODUCTION_CONNECTOR_REGISTRY_INSTALLER_TRUST_BOUNDARY;
  readonly execution_boundary: TBoundary;
  readonly algorithm: typeof FLOODGATE_V7_PRODUCTION_CONNECTOR_REGISTRY_INSTALLER_ALGORITHM;
  readonly registry: Readonly<{
    readonly managed_directory_mode: "0700";
    readonly runs_directory_mode: "0700";
    readonly registry_mode: "0600";
    readonly registry_nlink: 1;
    readonly publication: "staged-registry-fsynced-hard-link-no-clobber-final-directory-fsynced-staging-unlinked-directory-fsynced-v1";
    readonly durability: "registry-published-and-staging-removal-durable";
  ...
  readonly nonclaims: Readonly<{
    ...
    readonly registry_path_disclosed: false;
...
export interface FloodgateV7ProductionConnectorRegistryInstallerDependenciesForTests {
  ...
  readonly failpointForTests?: (
    phase: FloodgateV7ProductionConnectorRegistryInstallerFailpoint,
  ) => void;
  readonly observeFailureForTests?: (failure: unknown) => void;
}

export class FloodgateV7ProductionConnectorRegistryInstallerError extends Error {
  readonly phase: FloodgateV7ProductionConnectorRegistryInstallerPhase;
  readonly durability: FloodgateV7ProductionConnectorRegistryInstallerDurability;
  readonly registry_may_have_been_created: boolean;
  readonly retry_disposition: FloodgateV7ProductionConnectorRegistryInstallerRetryDisposition;

  constructor(
    phase: FloodgateV7ProductionConnectorRegistryInstallerPhase,
    durability: FloodgateV7ProductionConnectorRegistryInstallerDurability,
    registryMayHaveBeenCreated: boolean,
    retryDisposition: FloodgateV7ProductionConnectorRegistryInstallerRetryDisposition,
  ) {
    super(
      "Floodgate v7 production connector registry installation failed; inspect reconciliation metadata",
    );
    this.name = "FloodgateV7ProductionConnectorRegistryInstallerError";
    this.phase = phase;
    ...
    objectDefineProperty(this, "stack", {
      ...
      writable: false,
      value:
        "FloodgateV7ProductionConnectorRegistryInstallerError: installation failed; inspect reconciliation metadata",
    });
    objectFreeze(this);
  }
}
...
function captureDependencies(
  value: FloodgateV7ProductionConnectorRegistryInstallerDependenciesForTests,
  ...
  return frozenRecord({
    ...
    closeFileHandleForTests: descriptors.closeFileHandleForTests?.value as
      ((handle: fs.promises.FileHandle) => Promise<void>) | undefined,
...
function phaseForFailpoint(
  failpoint: FloodgateV7ProductionConnectorRegistryInstallerFailpoint,
): FloodgateV7ProductionConnectorRegistryInstallerPhase {
  switch (failpoint) {
...
async function inspectExistingChain(
  ...
  return FLOODGATE_V7_PRODUCTION_CONNECTOR_REGISTRY_ROOT_RELATIVE_COMPONENTS.length;
}
...
async function ensureRunsDirectory(
  ...
  const registryRoot = state.parentReference;
  if (registryRoot === null) {
    throw new NativeError("installer registry root descriptor is unavailable");
  }
  await revalidateDirectory(registryRoot, dependencies.effectiveUserId);
  const runsPath = appendFixedPathComponent(
    registryRoot.filePath,
    FLOODGATE_V7_PRODUCTION_CONNECTOR_RUNS_BASENAME,
  );
  const before = await lstatMaybe(runsPath);
  if (before === null) {
    state.directoryCreationAttempted = true;
    try {
      await capturedFs.mkdir(runsPath, { mode: 0o700 });
    } catch (error) {
      if (!isErrno(error, "EEXIST")) throw error;
    }
  }
  const runsReference = await openAndValidateDirectory(
    runsPath,
    dependencies.effectiveUserId,
    "managed-exact-0700",
  );
  appendDirectoryReference(state, runsReference);
  await revalidateDirectory(registryRoot, dependencies.effectiveUserId);
  if (before === null) {
    await runsReference.handle.sync();
    await registryRoot.handle.sync();
    invokeFailpoint(state, dependencies, "after-managed-directory-created");
  }
}
...
async function prepareNamespace(
  ...
  const registryRootReference = state.parentReference;
  await ensureRunsDirectory(dependencies, state);
  state.parentReference = registryRootReference;
  ...
  if (final !== null) {
    ...
    throw new NativeError(
      "production connector registry record already exists",
    );
  }
  ...
  if (staging !== null) {
    ...
    throw new NativeError(
      "production connector registry staging already exists",
    );
  }
...
async function createAndPublishRecord(
  ...
  if ((await lstatMaybe(state.finalPath)) !== null) {
    state.existingFinal = true;
    throw new NativeError("production connector registry record appeared");
  }
...
async function reconcileFailure(
  ...
  if (state.descriptorCloseFailedAfterFinalRevalidation) {
    return frozenRecord({
      durability: "registry-published-and-staging-removal-durable" as const,
      mayHaveCommitted: true,
      retryDisposition: "do-not-retry-existing-registry" as const,
    });
  }
  if (state.existingFinal && !state.linkSucceeded) {
    if (
      ...
      try {
        ...
        if (
          ...
          throw new NativeError(
            "installer existing-registry staging reconciliation is ambiguous",
          );
        }
      } catch {
        return frozenRecord({
    ...
    return frozenRecord({
      durability: state.directoryCreationAttempted
        ? ("managed-prefix-may-exist-existing-registry-not-adopted" as const)
    ...
    try {
      if (
        ...
        if (
          ...
          return frozenRecord({
            durability:
              "registry-published-and-staging-removal-durable" as const,
            mayHaveCommitted: true,
            retryDisposition: "manual-reconciliation-required" as const,
          });
        }
  ...
  if (
    ...
    try {
      ...
      if (
        ...
        await state.parentReference.handle.sync();
        return frozenRecord({
          durability: "parent-chain-durable-registry-absent" as const,
          mayHaveCommitted: false,
  ...
  return frozenRecord({
    durability: state.parentChainDurable
      ? ("parent-chain-durable-registry-absent" as const)
      : state.directoryCreationAttempted
        ? ("managed-prefix-may-exist-registry-absent" as const)
        : ("no-installation-change-established" as const),
    mayHaveCommitted: false,
...
>(
  boundary: TBoundary,
): Readonly<FloodgateV7ProductionConnectorRegistryInstallerReceipt<TBoundary>> {
  const production =
    boundary ===
    "production-fixed-current-euid-userinfo-home-production-connector-registry-installation";
  return frozenRecord({
    ...
    claim_boundary:
      FLOODGATE_V7_PRODUCTION_CONNECTOR_REGISTRY_INSTALLER_CLAIM_BOUNDARY,
    trust_boundary:
      FLOODGATE_V7_PRODUCTION_CONNECTOR_REGISTRY_INSTALLER_TRUST_BOUNDARY,
    execution_boundary: boundary,
    algorithm: FLOODGATE_V7_PRODUCTION_CONNECTOR_REGISTRY_INSTALLER_ALGORITHM,
    registry: frozenRecord({
      managed_directory_mode: "0700" as const,
      runs_directory_mode: "0700" as const,
      registry_mode: "0600" as const,
      registry_nlink: 1 as const,
      publication:
        "staged-registry-fsynced-hard-link-no-clobber-final-directory-fsynced-staging-unlinked-directory-fsynced-v1" as const,
      durability: "registry-published-and-staging-removal-durable" as const,
    ...
    nonclaims: frozenRecord({
      ...
      registry_path_disclosed: false as const,
...
>(
  ...
  dependencies: CapturedDependencies,
  boundary: TBoundary,
  recordBoundary: FloodgateV7ProductionConnectorRegistryExecutionBoundary,
...
export function installFloodgateV7ProductionConnectorRegistryCoreForTests(
  input: FloodgateV7ProductionConnectorRegistryInstallationInput,
  dependenciesValue: FloodgateV7ProductionConnectorRegistryInstallerDependenciesForTests,
): Promise<
  Readonly<
    FloodgateV7ProductionConnectorRegistryInstallerReceipt<"test-only-injected-current-euid-home-production-connector-registry-installation">
  >
> {
