...
def _validate_ratio(value: Any, label: str) -> int:
    ...
    if (
        ...
        raise FormalAbV2BenchmarkBoundRunnerError(f"{label} fraction differs")
    ...
    if (
        ...
        raise FormalAbV2BenchmarkBoundRunnerError(f"{label} decimal differs")
    return numerator
...
def _validate_public_analysis(value: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if type(attempt_index) is not int or attempt_index not in (0, 1):
        raise FormalAbV2BenchmarkBoundRunnerError(
            "formal public analysis attempt index differs"
        )
    ...
    if candidate_sha256 == stable_sha256:
        raise FormalAbV2BenchmarkBoundRunnerError(
            "formal public analysis weight identities match"
        )
    formal_protocol._require_exact_sha256(
