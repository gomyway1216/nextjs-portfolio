"""Benchmark-bound production bridge over the immutable P0 foundation.
...
import formal_paired_ab_v2_worker_benchmark as benchmark_core
import formal_paired_ab_v2_worker_benchmark_authority as benchmark_authority


PAIR_WORKER_CANDIDATES = benchmark_core.PAIR_WORKER_CANDIDATES
BENCHMARK_PAIRS_PER_ROUND = benchmark_core.BENCHMARK_PAIRS_PER_ROUND
BENCHMARK_GAMES_PER_ROUND = benchmark_core.BENCHMARK_GAMES_PER_ROUND
BENCHMARK_SEQUENCE = benchmark_core.BENCHMARK_SEQUENCE
REPETITIONS_PER_SETTING = benchmark_core.REPETITIONS_PER_SETTING
BENCHMARK_ROUND_RESULT_SCHEMA = benchmark_core.BENCHMARK_ROUND_RESULT_SCHEMA
BENCHMARK_RECEIPT_SCHEMA = benchmark_core.BENCHMARK_RECEIPT_SCHEMA
...
SELECTION_CONDITION = benchmark_core.SELECTION_CONDITION

_PINNED_BENCHMARK_REGISTRY_IDENTITY = (
    benchmark_authority.PINNED_BENCHMARK_REGISTRY_IDENTITY
)
_PINNED_FORMAL_READY_REGISTRY_IDENTITY = (
    benchmark_authority.PINNED_FORMAL_READY_REGISTRY_IDENTITY
)

_IMPLEMENTATION_PATHS = {
    "benchmark_orchestrator": "ml/formal_paired_ab_v2_worker_benchmark_bridge.py",
    "benchmark_selection_core": "ml/formal_paired_ab_v2_worker_benchmark.py",
    "benchmark_production_runner": "ml/run_formal_paired_ab_v2_worker_benchmark.py",
    "formal_production_runner": "ml/formal_paired_ab_v2_benchmark_bound_runner.py",
    "local_launcher": "ml/formal_paired_ab_local_launcher.py",
    "formal_protocol": "ml/formal_paired_ab_protocol.py",
    "formal_protocol_v2": "ml/formal_paired_ab_protocol_v2.py",
    "fresh_parent_accounting": "ml/fresh_qat_parent_accounting_v2.py",
    "fresh_qat_protocol": "ml/fresh_qat_protocol.py",
    ...
    "sfen_runtime": "ml/shogi-sfen.ts",
    "sfen_codec": "ml/shogi-sfen-codec.ts",
    "generate_moves": "src/components/game/ShogiImproved/GenerateMovesImproved.ts",
    "position": "src/components/game/ShogiImproved/KyokumenImproved.ts",
    "move_list": "src/components/game/ShogiImproved/MoveListImproved.ts",
    "transposition_entry": "src/components/game/ShogiImproved/TTEntryImproved.ts",
    "promotion_rules": "src/components/game/ShogiImproved/PromotionRulesImproved.ts",
    "shared_transposition_table": "src/components/game/ShogiImproved/sharedTT.ts",
    "shogi_types": "src/components/game/ShogiImproved/types.ts",
    "wasm_engine": "src/components/game/ShogiImproved/wasmEngine.ts",
...
FormalAbV2WorkerBenchmarkError = benchmark_core.FormalAbV2WorkerBenchmarkError
...
select_formal_ab_v2_pair_workers = benchmark_core.select_formal_ab_v2_pair_workers
...
run_formal_ab_v2_worker_benchmark_core_for_tests = (
    benchmark_core.run_formal_ab_v2_worker_benchmark_core_for_tests
)
...
def _reserve_private_run_output(
    home_root: str | Path,
    ...
    *,
    ...
    """Reserve one private run namespace before any irreversible work starts."""

    try:
        validated_id = legacy._require_exact_semantic_id(semantic_id, f"{label} ID")
        components = legacy._safe_relative_parts(
            output_directory, f"{label} output directory"
        )
    except ValueError as error:
        raise FormalAbV2WorkerBenchmarkError(str(error)) from error
    ...
    try:
        ...
        _verify_owned_directory(
            os.fstat(home_descriptor), f"{label} home", private=False
        )
        ...
        for index, component in enumerate(components):
            following = _open_or_create_output_directory(
                current,
                component,
                f"{label} output component {component}",
                private=index > 0,
            )
        ...
        try:
            ...
            raise FormalAbV2WorkerBenchmarkError(
                f"{label} output is already reserved; automatic rerun is forbidden"
            ) from error
        except OSError as error:
            raise FormalAbV2WorkerBenchmarkError(
                f"{label} output cannot be reserved safely"
            ) from error
        ...
        _verify_owned_directory(run_metadata, f"{label} run directory", private=True)
        ...
        if (path_metadata.st_dev, path_metadata.st_ino) != (
            ...
            raise FormalAbV2WorkerBenchmarkError(
                f"{label} output reservation identity differs"
            )
        reservation = {
            "semantic_id": validated_id,
            "label": label,
            "path": home.joinpath(*components, run_name),
...
def _reserve_worker_benchmark_output(
    ...
    reservation = _reserve_private_run_output(
        home_root,
        BENCHMARK_OUTPUT_DIRECTORY,
        benchmark_id,
...
def _close_private_run_output_reservation(reservation: dict[str, Any]) -> None:
    for field in ("run_descriptor", "parent_descriptor"):
        ...
        if type(descriptor) is int:
            try:
                ...
                pass
            reservation[field] = None


def _close_worker_benchmark_output_reservation(reservation: dict[str, Any]) -> None:
    _close_private_run_output_reservation(reservation)


def _verify_private_run_output_reservation(
    reservation: Mapping[str, Any],
) -> None:
    label = reservation["label"]
    run_descriptor = reservation["run_descriptor"]
    metadata = os.fstat(run_descriptor)
    _verify_owned_directory(metadata, f"{label} run directory", private=True)
    path_metadata = os.stat(
    ...
    if (metadata.st_dev, metadata.st_ino) != (
        ...
        raise FormalAbV2WorkerBenchmarkError(
            f"{label} output reservation no longer names its inode"
        )
...
def _verify_worker_benchmark_output_reservation(
    ...
    _verify_private_run_output_reservation(reservation)
...
def _run_pinned_worker_benchmark_core_for_tests(
    ...
    try:
        ...
        recaptured = validate_pinned_worker_benchmark_registry(repo_root)
        _require_exact_json(
            recaptured,
            captured,
...
def capture_formal_ready_registry(
    ...
    *,
    ...
    root = Path(repo_root).resolve(strict=True)
    # The bound receipt cannot nominate its own benchmark authority.  Resolve
    # the independently code-pinned trust root first, then authenticate its
    # exact READY bytes, implementation inventory, and enrolled artifacts.
    pinned_benchmark = validate_pinned_worker_benchmark_registry(root)
    relative = _registry_relative_path(root, registry_path)
    expected_identity = (
        None
        if expected_registry_identity is None
    ...
    try:
        ...
        bound_receipt = validate_bound_worker_benchmark_receipt(
            benchmark_payload,
            expected_registry=pinned_benchmark["registry"],
            expected_registry_identity=pinned_benchmark["registry_identity"],
        )
    except ValueError as error:
        raise FormalAbV2WorkerBenchmarkError(str(error)) from error
    _require_exact_json(
        bound_receipt["benchmark_registry"],
        pinned_benchmark["registry_identity"],
        "formal WASM worker benchmark trust root",
    )
    pinned_benchmark_values = {
        "match_binding": pinned_benchmark["match_binding_identity"],
        "dedicated_openings_manifest": pinned_benchmark[
            "dedicated_openings_manifest_identity"
        ],
        "openings_preflight_receipt": pinned_benchmark[
            "openings_preflight_receipt_identity"
        ],
        "candidate_weights": pinned_benchmark["assets"]["candidate_weights"],
        "stable_weights": pinned_benchmark["assets"]["stable_weights"],
    }
    receipt_benchmark_values = {
        key: bound_receipt[key] for key in pinned_benchmark_values
    }
    _require_exact_json(
        receipt_benchmark_values,
        pinned_benchmark_values,
        "formal WASM worker benchmark enrolled evidence",
    )
    if (
