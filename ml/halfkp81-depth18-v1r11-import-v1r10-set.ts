...
async function readSourceSnapshots(repositoryRoot: string) {
  ...
  const [selection, selectionManifest, terminalFault, engineBinary, engineEval, engineReceipt] =
    await Promise.all([
      read(path.join(SELECTION_ROOT, "hard-parents.jsonl"), SELECTION_ROOT, "v1r10 import selection", 7_268_777),
      read(path.join(SELECTION_ROOT, "hard-parents.manifest.json"), SELECTION_ROOT, "v1r10 import selection manifest", 3_234),
      read(path.join(SOURCE_ROOT, "teacher-terminal-fault.json"), SOURCE_ROOT, "v1r10 import terminal fault", 1_084),
      read(header.engine.binary.path, ASSET_ROOT, "v1r10 import engine binary", 700_048, false),
      read(header.engine.eval_file.path, ASSET_ROOT, "v1r10 import engine eval", 64_217_066, false),
      read(header.engine.receipt.path, repositoryRoot, "v1r10 import engine receipt", 654, false),
    ]);
  return Object.freeze({
    plan,
    selection,
