...
export function positionFromSfen(sfen: string): ParsedSfenPosition {
  ...
  for (let file = 1; file <= 9; file++) {
    for (let rank = 1; rank <= 9; rank++) position.ban[square(file, rank)] = EMPTY;
  }
  position.hand.fill(0);

  const rows = parts[0].split('/');
  if (rows.length !== 9) throw new Error(`SFEN board must have nine ranks: ${parts[0]}`);
  for (let rank = 1; rank <= 9; rank++) {
    ...
    for (let index = 0; index < row.length; index++) {
      ...
      if (token === '+') {
        promoted = true;
        piece = row[++index];
        if (!piece) throw new Error(`dangling promotion marker in SFEN rank ${rank}`);
      }
      const upper = piece.toUpperCase();
      const base = PIECE_KIND[upper];
      if (!base || file < 1 || file > 9) throw new Error(`invalid SFEN piece ${piece}`);
    ...
    if (file !== 0) throw new Error(`SFEN rank ${rank} does not contain nine squares`);
  }
  ...
  if (parts[2] !== '-') {
    ...
    for (const piece of parts[2]) {
      ...
      const upper = piece.toUpperCase();
      const base = PIECE_KIND[upper];
      if (!base || base === OU) throw new Error(`invalid SFEN hand piece ${piece}`);
      const copies = count ? Number.parseInt(count, 10) : 1;
      if (!Number.isInteger(copies) || copies <= 0) throw new Error(`invalid SFEN hand count ${count}`);
      const side = piece === upper ? SENTE : GOTE;
...
export function teToUsi(move: Te): string {
  ...
  if (move.from === 0) {
    ...
    if (!piece) throw new Error(`cannot encode dropped piece ${getKomashu(move.koma)}`);
...
export function resolveUsiMove(position: KyokumenImproved, usi: string): Te {
  ...
  if (drop) {
    ...
    matches = legal.filter(
      (move) => move.from === 0 && move.to === to && getKomashu(move.koma) === kind
    );
  } else {
    const boardMove = usi.match(/^([1-9])([a-i])([1-9])([a-i])(\+)?$/i);
    if (!boardMove) throw new Error(`invalid USI move: ${usi}`);
    const from = square(Number.parseInt(boardMove[1], 10), rankNumber(boardMove[2]));
    const to = square(Number.parseInt(boardMove[3], 10), rankNumber(boardMove[4]));
