...
class FormalPairedAbV2BenchmarkBoundRunnerTest(unittest.TestCase):
    ...
    def _public_analysis(self):
        denominator = runner.formal_protocol.SCORE_DENOMINATOR
        one_sided_lower = 800
        two_sided_lower = 780
        return {
            "schema": runner.formal_protocol.FORMAL_AB_V2_ANALYSIS_SCHEMA,
            ...
            "protocol_amendment_sha256": (
                runner.formal_protocol.FORMAL_AB_V2_AMENDMENT_SHA256
            ),
            "counts": {
                "pairs": runner.formal_protocol.PAIR_COUNT,
                "games": runner.formal_protocol.GAME_COUNT,
            },
            "point_score_rate": {
                "numerator": 810,
            ...
            "bootstrap": {
                "seed": runner.formal_protocol.BOOTSTRAP_SEED,
                "replicates": runner.formal_protocol.BOOTSTRAP_REPLICATES,
                "resampling_unit": "two-game-color-swapped-opening-pair",
                "one_sided_95_lower_rank": (
                    runner.formal_protocol.ONE_SIDED_95_LOWER_RANK
                ),
                "one_sided_95_lower": {
                ...
                "two_sided_95_lower_rank": (
                    runner.formal_protocol.TWO_SIDED_95_LOWER_RANK
                ),
                "two_sided_95_lower": {
    ...
    def test_public_analysis_validator_rejects_nested_or_semantic_drift(self):
        ...
        mutations = {
            ...
            "bootstrap-constant": lambda value: set_nested(
                value,
                ("bootstrap", "seed"),
                runner.formal_protocol.BOOTSTRAP_SEED + 1,
            ),
        ...
        for label, mutate in mutations.items():
            with self.subTest(label=label):
                ...
                with self.assertRaises(ValueError):
                    runner._validate_public_analysis(analysis)

        rerun = self._public_analysis()
        rerun["attempt_index"] = 1
        rerun["rerun_authorization_sha256"] = "7" * 64
        self.assertEqual(runner._validate_public_analysis(rerun), rerun)
    ...
    def test_cli_sanitizes_all_analysis_failures_without_traceback_or_path(self):
        ...
        for label, unsafe in unsafe_values.items():
            with self.subTest(label=label):
                ...
                self.assertEqual(stdout, "")
                self.assertEqual(
                    json.loads(stderr), runner._cli_stop("formal-run-failed-closed")
                )
    ...
    def test_cli_catches_canonical_serialization_failure(self):
        ...
        with mock.patch.object(
            runner, "_validate_public_analysis", return_value=unsafe
        ):
            code, stdout, stderr = self._run_cli(
    ...
    def test_main_wires_the_fixed_runner_and_analyzer_once(self):
        ...
        with (
            mock.patch.object(
                runner, "run_pinned_ready_wasm_pairs", return_value=private_result
            ) as run,
            mock.patch.object(
