...
import publish_halfkp81_depth18_yaneura_only_v1r4_plan as PUBLISHER  # noqa: E402


NEW_REVISION = "a" * 40
TRACKED_PLAN = ML_DIR / "halfkp81-hard-depth18-yaneura-only-v1r4-plan.json"
FAILED_V1R3_PLAN = Path(
    PROTOCOL.EXPECTED_FAILED_V1R3["teacher_plan"]["path"]
)
HAS_FAILED_V1R3_LOCAL_ARTIFACTS = (
    FAILED_V1R3_PLAN.is_file()
    and Path(
        PROTOCOL.EXPECTED_FAILED_V1R3["preflight_namespace"]["directory"]
    ).is_dir()
    and Path(
        PROTOCOL.EXPECTED_FAILED_V1R3["launch"]["namespace"]
    ).is_dir()
)
...
class PublishYaneuraOnlyV1R4PlanTests(unittest.TestCase):
    def test_build_authenticates_v1r3_boundary_and_rechecks_main(self) -> None:
        ...
        with (
            ...
            mock.patch.object(
                PUBLISHER, "_verify_failed_v1r3_artifacts"
            ) as predecessor,
            mock.patch.object(
                PUBLISHER, "_verify_failed_v1r3_quiescent"
            ) as quiescent,
        ...
        self.assertEqual(plan["predecessor_v1r3"]["completed_parents"], 512)
        self.assertEqual(plan["predecessor_v1r3"]["teacher_rows"], 6_134)
        self.assertEqual(plan["predecessor_v1r3"]["reuse_teacher_rows"], 0)
        self.assertEqual(
            plan["technical_recovery"][
                "preflight_validator_requested_multipv"
            ]["expected"],
            "min(12, legal_moves_count)",
        )

    @unittest.skipUnless(
        HAS_FAILED_V1R3_LOCAL_ARTIFACTS,
        "requires the private failed-v1r3 artifacts on the originating Mac",
    )
    def test_failed_v1r3_actual_artifacts_and_quiescence_authenticate(
        self,
    ) -> None:
        PUBLISHER._verify_failed_v1r3_artifacts(
    ...
    def test_failed_revision_is_rejected_before_git(self) -> None:
        with mock.patch.object(
            ...
            with self.assertRaisesRegex(
                PUBLISHER.YaneuraOnlyV1R4PublicationError,
                "failed v1r3 source revision is forbidden",
            ):
                PUBLISHER._verify_merged_revision(
                    PROTOCOL.FAILED_V1R3_REVISION, Path("/tmp/repo")
                )
    ...
    def test_publisher_rejects_v1r3_or_alternate_namespace(self) -> None:
        plan = {
            "schema": PROTOCOL.TEACHER_PLAN_SCHEMA,
            "outputs": copy.deepcopy(PROTOCOL.EXPECTED_RUNTIME_OUTPUTS),
        }
        with self.assertRaisesRegex(
            ...
            PUBLISHER.publish_teacher_plan(
                plan,
                output_directory=PROTOCOL.EXPECTED_FAILED_V1R3[
                    "formal_namespace"
                ]["directory"],
            )
