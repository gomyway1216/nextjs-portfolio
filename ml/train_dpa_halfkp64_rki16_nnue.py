#!/usr/bin/env python3
"""Fixed two-epoch streaming trainer for DPA-HalfKP64-RKI16.
...
CHECKPOINT_FAMILY = "dpa-halfkp64-rki16-scratch-v1"
...
def train(
    ...
    *,
    ...
    model = DpaHalfkp64Rki16NNUE().to(device)
    ...
    for epoch in range(completed_epoch + 1, EPOCHS + 1):
        ...
        for batch in mixed_batch_stream(
            ...
            if not bool(torch.isfinite(loss)):
                raise RuntimeError("DPA-HalfKP64-RKI16 produced a non-finite loss")
            loss.backward()
            optimizer.step()
            batches += 1
