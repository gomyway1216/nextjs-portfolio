/**
 * Shichinarabe AI self-play A/B harness.
 *
 * Mirrors scripts/daifugo-ai-match.ts: run N complete headless games with the
 * REAL rule engine (gameLogic.applyAction) and the REAL AI
 * (decideShichinarabeAction), with a seeded deterministic RNG and seat
 ...
 *   impl        'cur'  -> current production ShichinarabeAI.ts (fair card counting)
 *               'base' -> frozen pre-fair-play copy (scripts/shichinarabe-ai-baseline.ts,
 *                         still reads opponents' concealed hands)
 *   difficulty  easy | medium | hard | expert | master
 *
 * Usage:
 *   node -r tsx/cjs scripts/shichinarabe-ai-match.ts --games 1000 --seed 1 \
 *     --cand cur:hard --base base:hard
 *   node -r tsx/cjs scripts/shichinarabe-ai-match.ts --games 1000 --cand cur:hard --base cur:medium
 *
...
import { applyAction, createInitialShichinarabeState } from '../src/components/game/Shichinarabe/gameLogic';
import {
  decideShichinarabeAction as decideCurrent,
  type ShichinarabeAIDifficulty,
  type ShichinarabeAIDecision,
} from '../src/components/game/Shichinarabe/ShichinarabeAI';
import { decideShichinarabeAction as decideBaseline } from './shichinarabe-ai-baseline';
import type { ShichinarabeAction, ShichinarabeNetworkState } from '../src/components/game/Shichinarabe/multiplayerTypes';
...
type DecideFn = (
  ...
  playerId: string,
  difficulty: ShichinarabeAIDifficulty
...
const DIFFICULTIES: ShichinarabeAIDifficulty[] = ['easy', 'medium', 'hard', 'expert', 'master'];
...
function parseArgs(argv: string[]): MatchConfig {
  ...
  return {
    ...
    players,
    candSeats,
    cand: parsePlayerSpec(argMap.get('cand'), 'cur:hard'),
    base: parsePlayerSpec(argMap.get('base'), 'base:hard'),
    seed: parseIntArg(argMap.get('seed'), 1),
    // Matches the vs-AI UI default (ShichinarabeVsAI.tsx uses maxPasses: 3).
    maxPasses: Math.max(1, parseIntArg(argMap.get('maxPasses'), 3)),
    maxSteps: parseIntArg(argMap.get('maxSteps'), 2000),
...
interface GameOutcome {
  /** Final place (1 = winner/best) per seat id, from the engine's resultOrder. */
...
function playOneGame(
  ...
  let state = createInitialShichinarabeState(seatIds, { maxPasses: config.maxPasses });
  ...
  while (!state.finished && steps < config.maxSteps) {
    ...
    const action: ShichinarabeAction = decision.type === 'play'
      ? { actionId: `g${steps}`, type: 'play', playerId: pid, cardId: decision.cardId, timestamp: 0 }
  ...
  state.resultOrder.forEach((pid, idx) => placeBySeat.set(pid, idx + 1));
  return { placeBySeat, steps, anomalies };
}
...
function main(): void {
  ...
  if (!config.quiet) {
    console.log('[shichinarabe-ai-match] config:', {
      games: config.games,
  ...
  if (totalAnomalies > 0) {
    console.error(`[shichinarabe-ai-match] WARNING: ${totalAnomalies} illegal AI actions were auto-passed`);
    process.exitCode = 1;
  }
}
