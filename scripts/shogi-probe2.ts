/**
 * Probe the reported game where the climbing silver won the 3四 pawn via 3六→2五→3四.
 */
import { generateLegalMoves } from '../src/components/game/Shogi/GenerateMoves';
import { createInitialPosition } from '../src/components/game/Shogi/InitialPosition';
import { getBestMove } from '../src/components/game/Shogi/ShogiAI';
import { FU, GOTE, SENTE, Te, getKomashu } from '../src/components/game/Shogi/types';
import type { Difficulty } from '../src/components/game/common/types';

const k = createInitialPosition();
k.teban = SENTE;
const history: Te[] = [];
let moveNumber = 1;

function play(fs: number, fd: number, ts: number, td: number, promote = false): void {
  const legal = generateLegalMoves(k);
  const te = legal.find(
    (m) => m.from.suji === fs && m.from.dan === fd && m.to.suji === ts && m.to.dan === td && m.promote === promote
  );
  if (!te) throw new Error(`illegal ${fs}${fd}->${ts}${td} at move ${moveNumber}`);
  k.move(te);
  history.push(te);
  k.teban = k.teban === SENTE ? GOTE : SENTE;
  moveNumber++;
}

function playDrop(komashu: number, ts: number, td: number): void {
  const legal = generateLegalMoves(k);
  const te = legal.find(
    (m) => m.from.suji === 0 && m.to.suji === ts && m.to.dan === td && getKomashu(m.koma) === komashu
  );
  if (!te) throw new Error(`illegal drop at move ${moveNumber}`);
  k.move(te);
  history.push(te);
  k.teban = k.teban === SENTE ? GOTE : SENTE;
  moveNumber++;
}

function askAI(label: string): void {
  const t0 = Date.now();
  const move = getBestMove(k, GOTE, (process.argv[2] ?? 'hard') as Difficulty, moveNumber, history);
  const ms = Date.now() - t0;
  if (!move) {
    console.log(`${label}: NO MOVE`);
    return;
  }
  const from = move.from.suji === 0 ? '打' : `${move.from.suji}${move.from.dan}`;
  console.log(`${label}: AI -> ${from}->${move.to.suji}${move.to.dan}${move.promote ? '成' : ''} (${ms}ms)`);
}

// The reported game
play(2, 7, 2, 6); // 1 ▲2六歩
play(8, 3, 8, 4); // 2 △8四歩
play(2, 6, 2, 5); // 3 ▲2五歩
play(8, 4, 8, 5); // 4 △8五歩
play(6, 9, 7, 8); // 5 ▲7八金
play(4, 1, 3, 2); // 6 △3二金
play(2, 5, 2, 4); // 7 ▲2四歩
play(2, 3, 2, 4); // 8 △同歩
play(2, 8, 2, 4); // 9 ▲同飛
playDrop(FU, 2, 3); // 10 △2三歩打
play(2, 4, 2, 8); // 11 ▲2八飛
play(8, 5, 8, 6); // 12 △8六歩
play(8, 7, 8, 6); // 13 ▲同歩
play(8, 2, 8, 6); // 14 △同飛
playDrop(FU, 8, 7); // 15 ▲8七歩打
play(8, 6, 8, 5); // 16 △8五飛
play(3, 9, 3, 8); // 17 ▲3八銀
play(7, 1, 6, 2); // 18 △6二銀
play(3, 8, 2, 7); // 19 ▲2七銀
play(1, 3, 1, 4); // 20 △1四歩
play(2, 7, 3, 6); // 21 ▲3六銀

console.log('--- move 22: silver on 3六 heading to 2五. Reported: △3四歩 ---');
askAI('move 22');

play(3, 3, 3, 4); // 22 △3四歩 (as reported)
play(3, 6, 2, 5); // 23 ▲2五銀

console.log('--- move 24: ▲2五銀 attacks the (undefended) 3四 pawn. Reported: △4二銀 ---');
askAI('move 24');

play(3, 1, 4, 2); // 24 △4二銀 (as reported)
play(2, 5, 3, 4); // 25 ▲3四銀 (pawn taken)

console.log('--- move 26: silver took 3四. Reported: △5五角 ---');
askAI('move 26');
