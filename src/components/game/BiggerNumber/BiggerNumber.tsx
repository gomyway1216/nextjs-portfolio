...
import { BiggerNumberVsAI } from './BiggerNumberVsAI';
import { BiggerNumberOnline } from './BiggerNumberOnline';
import {
...
function BiggerNumberContent() {
  useFeatureLifecycle('game.bigger-number');
  const [mode, setMode] = useState<Mode>('menu');
  const { language } = useGameLanguage();
  const t = getGameTranslation('bigger-number', language);

  if (mode === 'ai') return <BiggerNumberVsAI onBackToMenu={() => setMode('menu')} />;
  if (mode === 'online') return <BiggerNumberOnline onBackToMenu={() => setMode('menu')} />;
