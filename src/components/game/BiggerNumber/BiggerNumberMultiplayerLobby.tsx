/**
 * Bigger Number — multiplayer lobby (menu / create / join / waiting).
 */

'use client';

import React, { useMemo, useState } from 'react';
import type { UseBiggerNumberMultiplayerReturn } from './useBiggerNumberMultiplayer';
import { RuleConfigEditor } from './RuleConfigEditor';
import { DEFAULT_RULES, type BiggerNumberRules } from './types';
...
export function BiggerNumberMultiplayerLobby({
  multiplayer,
  onGameStart,
}: BiggerNumberMultiplayerLobbyProps) {
  ...
  const roomRules = (room?.rules as BiggerNumberRules | undefined) ?? null;
  React.useEffect(() => {
  ...
  if (context.lobbyState === 'waiting' || context.lobbyState === 'ready') {
    return (
      <div style={containerStyle}>
        ...
        {/* Show rules to both players (read-only for non-host) */}
        <RuleConfigEditor
          rules={rules}
          onChange={setRules}
          language={language}
