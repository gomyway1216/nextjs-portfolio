/**
 * Shichinarabe (七並べ) - Multiplayer Lobby
 */

'use client';

import React, { useMemo, useState } from 'react';
import type { UseShichinarabeMultiplayerReturn } from './useShichinarabeMultiplayer';

interface ShichinarabeMultiplayerLobbyProps {
  multiplayer: UseShichinarabeMultiplayerReturn;
  onGameStart: () => void;
}

export function ShichinarabeMultiplayerLobby({ multiplayer, onGameStart }: ShichinarabeMultiplayerLobbyProps) {
  ...
  if (context.lobbyState === 'idle' && mode === 'menu') {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1.25rem', padding: '1rem' }}>
        ...
        <h2 style={{ fontSize: '1.75rem', fontWeight: 'bold', color: '#fbbf24', margin: 0 }}>Shichinarabe (七並べ)</h2>
