 ...
 *
 * V10 additions:
 * - Packed Transposition Table (TT) that stores move keys instead of `Te` objects to reduce GC and search deeper.
 * - Keeps V9’s opening-aware root move ordering + root safety ordering + evaluation cache.
 *
 ...
 */
export class ShogiAIImprovedV10 {
  ...
  private killer1: number[] = new Array(ShogiAIImprovedV10.MAX_PLY).fill(0);
  private killer2: number[] = new Array(ShogiAIImprovedV10.MAX_PLY).fill(0);
  ...
  constructor(tt: TranspositionTableImprovedPacked = new TranspositionTableImprovedPacked()) {
    ...
    this.evalCacheKeyV1 = new Int32Array(ShogiAIImprovedV10.EVAL_CACHE_SIZE);
    this.evalCacheValV1 = new Int32Array(ShogiAIImprovedV10.EVAL_CACHE_SIZE);
    this.evalCacheKeyV2 = new Int32Array(ShogiAIImprovedV10.EVAL_CACHE_SIZE);
    this.evalCacheValV2 = new Int32Array(ShogiAIImprovedV10.EVAL_CACHE_SIZE);
    this.evalCacheKeyV3 = new Int32Array(ShogiAIImprovedV10.EVAL_CACHE_SIZE);
    this.evalCacheValV3 = new Int32Array(ShogiAIImprovedV10.EVAL_CACHE_SIZE);

    this.evalCacheKeyV1.fill(ShogiAIImprovedV10.EVAL_CACHE_SENTINEL);
    this.evalCacheKeyV2.fill(ShogiAIImprovedV10.EVAL_CACHE_SENTINEL);
    this.evalCacheKeyV3.fill(ShogiAIImprovedV10.EVAL_CACHE_SENTINEL);
  }
  ...
  private evaluateSenteCached(k: KyokumenImproved): number {
    ...
    const index = key & (ShogiAIImprovedV10.EVAL_CACHE_SIZE - 1);
  ...
  private recordKiller(ply: number, key: number): void {
    ...
    if (ply < 0 || ply >= ShogiAIImprovedV10.MAX_PLY) return;
  ...
  private quiescence(k: KyokumenImproved, alpha: number, beta: number, ply: number, depthLeft: number): number {
    ...
    try {
      ...
      if (allMoves.length === 0) {
        ...
        return inCheck ? -ShogiAIImprovedV10.MATE + ply : 0;
      }
  ...
  private search(k: KyokumenImproved, depthLeft: number, alpha: number, beta: number, ply: number): number {
    ...
    try {
      ...
      const alphaMate = -ShogiAIImprovedV10.MATE + ply;
      if (alpha < alphaMate) alpha = alphaMate;
      const betaMate = ShogiAIImprovedV10.MATE - ply;
      ...
      if (moves.length === 0) {
        ...
        return parentInCheck ? -ShogiAIImprovedV10.MATE + ply : 0;
      }
  ...
  getNextTe(k: KyokumenImproved, tesu: number = 0, options: ShogiAISearchOptions = {}): Te | null {
    ...
    let bestScore = -ShogiAIImprovedV10.INFINITE;
    ...
    for (let depth = 1; depth <= maxDepth; depth++) {
      try {
        ...
        const alpha0 = useAspiration ? bestScore - this.aspirationWindow : -ShogiAIImprovedV10.INFINITE;
        const beta0 = useAspiration ? bestScore + this.aspirationWindow : ShogiAIImprovedV10.INFINITE;
        ...
        if (useAspiration && (score <= alpha0 || score >= beta0)) {
          ...
          this.resetRootBest();
          score = this.search(position, depth, -ShogiAIImprovedV10.INFINITE, ShogiAIImprovedV10.INFINITE, 0);
        }
        ...
        if (bestScore >= ShogiAIImprovedV10.MATE - 10_000) break;
      } catch (e) {
    ...
    if (options.debug) {
      ...
      console.log(
        `[ShogiAIImprovedV10] depth=${completedDepth}/${maxDepth} score=${bestScore} nodes=${this.node} leaves=${this.leaf} time=${Math.round(elapsed)}ms`
      );
    }
...
const sharedAIV10 = new ShogiAIImprovedV10();
 ...
 */
export function getBestMoveV10(k: KyokumenImproved, teban: number, difficulty: Difficulty): Te | null {
  // The UI passes `teban` explicitly; keep the position consistent.
  k.setTeban(teban);
  return sharedAIV10.getNextTe(k, 0, { difficulty });
}
