 ...
 * - Lightweight SEE-ish ordering:
 *   - for drops/captures, prefer defended moves and penalize obviously hanging moves using cheap attack scans.
 *   - caches "least attacker" values per node to keep overhead tiny.
 * - Stronger endgame conversion:
 *   - bounded check extensions on Expert/Master
 *   - limited quiet-check probing in quiescence on Expert/Master
 * - Small repetition contempt on Level 4/5 to reduce aimless repetition when clearly ahead/behind.
 * - Conservative quiescence delta pruning on Level 4/5 to improve effective depth under tight time budgets.
 *
 ...
 */
export class ShogiAIImprovedV17 {
  ...
  private killer1: number[] = new Array(ShogiAIImprovedV17.MAX_PLY).fill(0);
  private killer2: number[] = new Array(ShogiAIImprovedV17.MAX_PLY).fill(0);
  ...
  private moveLists: MoveListImproved[] = Array.from(
    { length: ShogiAIImprovedV17.MAX_PLY },
  ...
  // but we call it multiple times per node when ordering many moves.
  //
  // We cache per (ply, target-square, defender-side) and invalidate by bumping an epoch counter per node.
  private static readonly ATTACK_CACHE_SQUARES = 256; // 0..255 is enough for our 0x?? indexing
  private static readonly ATTACK_CACHE_INF = 0x7fffffff;

  private attackEpochByPly = new Int32Array(ShogiAIImprovedV17.MAX_PLY);
  private attackStampSente = new Int32Array(ShogiAIImprovedV17.MAX_PLY * ShogiAIImprovedV17.ATTACK_CACHE_SQUARES);
  private attackStampGote = new Int32Array(ShogiAIImprovedV17.MAX_PLY * ShogiAIImprovedV17.ATTACK_CACHE_SQUARES);
  private attackValSente = new Int32Array(ShogiAIImprovedV17.MAX_PLY * ShogiAIImprovedV17.ATTACK_CACHE_SQUARES);
  private attackValGote = new Int32Array(ShogiAIImprovedV17.MAX_PLY * ShogiAIImprovedV17.ATTACK_CACHE_SQUARES);
  ...
  constructor(tt: TranspositionTableImprovedPacked = new TranspositionTableImprovedPacked()) {
    ...
    this.evalCacheKeyV1 = new Int32Array(ShogiAIImprovedV17.EVAL_CACHE_SIZE);
    this.evalCacheValV1 = new Int32Array(ShogiAIImprovedV17.EVAL_CACHE_SIZE);
    this.evalCacheKeyV2 = new Int32Array(ShogiAIImprovedV17.EVAL_CACHE_SIZE);
    this.evalCacheValV2 = new Int32Array(ShogiAIImprovedV17.EVAL_CACHE_SIZE);
    this.evalCacheKeyV3 = new Int32Array(ShogiAIImprovedV17.EVAL_CACHE_SIZE);
    this.evalCacheValV3 = new Int32Array(ShogiAIImprovedV17.EVAL_CACHE_SIZE);

    this.evalCacheKeyV1.fill(ShogiAIImprovedV17.EVAL_CACHE_SENTINEL);
    this.evalCacheKeyV2.fill(ShogiAIImprovedV17.EVAL_CACHE_SENTINEL);
    this.evalCacheKeyV3.fill(ShogiAIImprovedV17.EVAL_CACHE_SENTINEL);
  }
  ...
  private evaluateSenteCached(k: KyokumenImproved): number {
    ...
    const index = key & (ShogiAIImprovedV17.EVAL_CACHE_SIZE - 1);
  ...
  private recordKiller(ply: number, key: number): void {
    ...
    if (ply < 0 || ply >= ShogiAIImprovedV17.MAX_PLY) return;
  ...
  private leastAttackerValueCached(k: KyokumenImproved, target: number, defender: number, ply: number): number {
    ...
    if ((stamp[index] | 0) === epoch) {
      ...
      return cached === ShogiAIImprovedV17.ATTACK_CACHE_INF ? Infinity : cached;
    }

    const computed = GenerateMovesImproved.getLeastAttackerValue(k, target, defender);
    const stored = Number.isFinite(computed) ? ((computed | 0) & 0x7fffffff) : ShogiAIImprovedV17.ATTACK_CACHE_INF;
    stamp[index] = epoch;
    val[index] = stored;
    return stored === ShogiAIImprovedV17.ATTACK_CACHE_INF ? Infinity : stored;
  }

  /**
   * Detects a "discovered" sliding attack on `to` that becomes available if the piece on `from` moves away.
   *
   * This is a cheap supplement to `getLeastAttackerValue()` which can miss attacks that are currently blocked
   * by the moving piece itself (e.g. moving off a file/diagonal that reveals an enemy rook/bishop).
   *
   * Returns the absolute value of the revealed enemy attacker, or `Infinity` if none is found.
   */
  private discoveredSliderAttackerValue(k: KyokumenImproved, from: number, to: number, defender: number): number {
    ...
    const dSuji = fromSuji - toSuji;
    const dDan = fromDan - toDan;
    if (dSuji === 0 && dDan === 0) return Infinity;
    ...
    if (dSuji === 0) {
      ...
      stepSuji = dSuji > 0 ? 1 : -1;
      stepDan = 0;
    } else if (Math.abs(dSuji) === Math.abs(dDan)) {
      stepSuji = dSuji > 0 ? 1 : -1;
      stepDan = dDan > 0 ? 1 : -1;
    } else {
      return Infinity;
    ...
    // Ensure there is nothing between `to` and `from` (otherwise `from` is not the blocker we remove).
    let suji = toSuji + stepSuji;
    let dan = toDan + stepDan;
    while (suji !== fromSuji || dan !== fromDan) {
      if (suji < 1 || suji > 9 || dan < 1 || dan > 9) return Infinity;
      const p = k.get((suji << 4) + dan);
      if (p !== EMPTY) return Infinity;
      suji += stepSuji;
      dan += stepDan;
    }

    // Scan beyond `from` to find a potential enemy slider on the same ray.
    suji = fromSuji + stepSuji;
    dan = fromDan + stepDan;
    while (suji >= 1 && suji <= 9 && dan >= 1 && dan <= 9) {
      const p = k.get((suji << 4) + dan);
      if (p === EMPTY) {
        suji += stepSuji;
        dan += stepDan;
      ...
      if (isSelf(defender, p)) break;

      const pieceType = getKomashu(p);
      const diagonal = stepSuji !== 0 && stepDan !== 0;

      if (diagonal) {
        // Bishop / Horse.
        if (pieceType === KA || pieceType === UM) return Math.abs(komaValue[p]) | 0;
      } else {
        // Rook / Dragon.
        if (pieceType === HI || pieceType === RY) return Math.abs(komaValue[p]) | 0;

        // Lance only attacks forward on files.
        if (pieceType === KY && stepSuji === 0) {
          const attackerSide = p & (SENTE | GOTE);
          const attackerToTargetDanStep = -stepDan;
          if (attackerSide === SENTE && attackerToTargetDanStep === -1) return Math.abs(komaValue[p]) | 0;
          if (attackerSide === GOTE && attackerToTargetDanStep === 1) return Math.abs(komaValue[p]) | 0;
        }
      }
      break;
    }
  ...
  private scoreMove(k: KyokumenImproved, te: Te, ply: number, ttMoveKey: number, ttSecondMoveKey: number): number {
    ...
    // SEE-lite ordering:
    // - Capturing with a much more valuable piece is often tactically risky in shogi.
    // - We don't want to prune sacrifices, only order away clearly hanging captures that win little material.
    if (te.from !== 0 && te.capture !== EMPTY) {
      const attackerValue = Math.abs(komaValue[te.koma]) | 0;
      const victimValue = Math.abs(komaValue[te.capture]) | 0;

      if (attackerValue >= 700 && victimValue + 200 < attackerValue) {
        ...
        if (distToEnemyKing > 2) {
          let enemyLeastAttacker = this.leastAttackerValueCached(k, te.to, k.teban, ply);
          const discovered = this.discoveredSliderAttackerValue(k, te.from, te.to, k.teban);
          if (discovered < enemyLeastAttacker) enemyLeastAttacker = discovered;
          ...
          if (Number.isFinite(enemyLeastAttacker)) {
            ...
            const defendedByCheaperOrEqual =
              Number.isFinite(selfLeastDefender) && (selfLeastDefender | 0) <= (enemyLeastAttacker | 0);

            if (!defendedByCheaperOrEqual) {
              const penalty = Math.min(160_000, attackerValue * 60);
              score -= penalty;
            }
          }
        }
		    ...
		    if (te.from === 0) {
		      ...
		      // - Penalize clearly hanging drops with a cheap attack/defense scan (SEE-lite).
		      ...
		      if (distToEnemyKing > 2) {
		        ...
		        if (Number.isFinite(enemyLeastAttacker)) {
		          ...
		          if (Number.isFinite(selfLeastDefender)) {
		            ...
		            const penalty = pieceValue <= 100 ? 20_000 : Math.min(180_000, pieceValue * 60);
		            score -= penalty;
		          }
		        }
		      }
      ...
      if (enemyKing > 0) {
	    ...
	    // SEE-lite for quiet moves: avoid hanging big pieces for no reason.
	    if (te.from !== 0 && te.capture === EMPTY && !te.promote) {
	      const movedValue = Math.abs(komaValue[te.koma]) | 0;
	      if (movedValue >= 700) {
	        const distToEnemyKing =
	          enemyKing > 0
	            ? Math.abs((te.to >> 4) - (enemyKing >> 4)) + Math.abs((te.to & 0x0f) - (enemyKing & 0x0f))
	        ...
	        if (distToEnemyKing > 2) {
	          ...
	          if (Number.isFinite(enemyLeastAttacker)) {
	            const selfLeastDefender = this.leastAttackerValueCached(k, te.to, this.otherSide(k.teban), ply);
	            if (!Number.isFinite(selfLeastDefender)) {
	              const penalty = Math.min(140_000, movedValue * 40);
	              score -= penalty;
	            }
	          }
	        }
  ...
  private quiescence(k: KyokumenImproved, alpha: number, beta: number, ply: number, depthLeft: number): number {
    ...
    try {
      ...
      if (allMoves.length === 0) {
        ...
        return inCheck ? -ShogiAIImprovedV17.MATE + ply : 0;
      }
  ...
  private search(k: KyokumenImproved, depthLeft: number, alpha: number, beta: number, ply: number): number {
    ...
    try {
      ...
      const alphaMate = -ShogiAIImprovedV17.MATE + ply;
      if (alpha < alphaMate) alpha = alphaMate;
      const betaMate = ShogiAIImprovedV17.MATE - ply;
      ...
      if (moves.length === 0) {
        ...
        return parentInCheck ? -ShogiAIImprovedV17.MATE + ply : 0;
      }
    ...
    let bestScore = -ShogiAIImprovedV17.INFINITE;
    ...
    for (let depth = 1; depth <= maxDepth; depth++) {
      try {
        ...
        const alpha0 = useAspiration ? bestScore - this.aspirationWindow : -ShogiAIImprovedV17.INFINITE;
        const beta0 = useAspiration ? bestScore + this.aspirationWindow : ShogiAIImprovedV17.INFINITE;
        ...
        if (useAspiration && (score <= alpha0 || score >= beta0)) {
          ...
          this.resetRootBest();
          score = this.search(position, depth, -ShogiAIImprovedV17.INFINITE, ShogiAIImprovedV17.INFINITE, 0);
        }
        ...
        if (bestScore >= ShogiAIImprovedV17.MATE - 10_000) break;
      } catch (e) {
    ...
    if (options.debug) {
      ...
      console.log(
        `[ShogiAIImprovedV17] depth=${completedDepth}/${maxDepth} score=${bestScore} nodes=${this.node} leaves=${this.leaf} time=${Math.round(elapsed)}ms`
      );
    }
...
const sharedAIV17 = new ShogiAIImprovedV17();
 ...
 */
export function getBestMoveV17(k: KyokumenImproved, teban: number, difficulty: Difficulty, tesu: number = 0): Te | null {
  // The UI passes `teban` explicitly; keep the position consistent.
  k.setTeban(teban);
  return sharedAIV17.getNextTe(k, tesu, { difficulty });
}
