 ...
 */
export class ShogiAIImprovedV18 {
  ...
  private killer1: number[] = new Array(ShogiAIImprovedV18.MAX_PLY).fill(0);
  private killer2: number[] = new Array(ShogiAIImprovedV18.MAX_PLY).fill(0);
  ...
  private moveLists: MoveListImproved[] = Array.from(
    { length: ShogiAIImprovedV18.MAX_PLY },
  ...
  private attackEpochByPly = new Int32Array(ShogiAIImprovedV18.MAX_PLY);
  private attackStampSente = new Int32Array(ShogiAIImprovedV18.MAX_PLY * ShogiAIImprovedV18.ATTACK_CACHE_SQUARES);
  private attackStampGote = new Int32Array(ShogiAIImprovedV18.MAX_PLY * ShogiAIImprovedV18.ATTACK_CACHE_SQUARES);
  private attackValSente = new Int32Array(ShogiAIImprovedV18.MAX_PLY * ShogiAIImprovedV18.ATTACK_CACHE_SQUARES);
  private attackValGote = new Int32Array(ShogiAIImprovedV18.MAX_PLY * ShogiAIImprovedV18.ATTACK_CACHE_SQUARES);
  ...
  constructor(tt: TranspositionTableImprovedPacked = new TranspositionTableImprovedPacked()) {
    ...
    this.evalCacheKeyV1 = new Int32Array(ShogiAIImprovedV18.EVAL_CACHE_SIZE);
    this.evalCacheValV1 = new Int32Array(ShogiAIImprovedV18.EVAL_CACHE_SIZE);
    this.evalCacheKeyV2 = new Int32Array(ShogiAIImprovedV18.EVAL_CACHE_SIZE);
    this.evalCacheValV2 = new Int32Array(ShogiAIImprovedV18.EVAL_CACHE_SIZE);
    this.evalCacheKeyV3 = new Int32Array(ShogiAIImprovedV18.EVAL_CACHE_SIZE);
    this.evalCacheValV3 = new Int32Array(ShogiAIImprovedV18.EVAL_CACHE_SIZE);

    this.evalCacheKeyV1.fill(ShogiAIImprovedV18.EVAL_CACHE_SENTINEL);
    this.evalCacheKeyV2.fill(ShogiAIImprovedV18.EVAL_CACHE_SENTINEL);
    this.evalCacheKeyV3.fill(ShogiAIImprovedV18.EVAL_CACHE_SENTINEL);
  }
  ...
  private evaluateSenteCached(k: KyokumenImproved): number {
    ...
    const index = key & (ShogiAIImprovedV18.EVAL_CACHE_SIZE - 1);
  ...
  private recordKiller(ply: number, key: number): void {
    ...
    if (ply < 0 || ply >= ShogiAIImprovedV18.MAX_PLY) return;
  ...
  private leastAttackerValueCached(k: KyokumenImproved, target: number, defender: number, ply: number): number {
    ...
    if ((stamp[index] | 0) === epoch) {
      ...
      return cached === ShogiAIImprovedV18.ATTACK_CACHE_INF ? Infinity : cached;
    }

    const computed = GenerateMovesImproved.getLeastAttackerValue(k, target, defender);
    const stored = Number.isFinite(computed) ? ((computed | 0) & 0x7fffffff) : ShogiAIImprovedV18.ATTACK_CACHE_INF;
    stamp[index] = epoch;
    val[index] = stored;
    return stored === ShogiAIImprovedV18.ATTACK_CACHE_INF ? Infinity : stored;
  }
  ...
  private quiescence(k: KyokumenImproved, alpha: number, beta: number, ply: number, depthLeft: number): number {
    ...
    try {
      ...
      if (allMoves.length === 0) {
        ...
        return inCheck ? -ShogiAIImprovedV18.MATE + ply : 0;
      }
  ...
  private search(k: KyokumenImproved, depthLeft: number, alpha: number, beta: number, ply: number): number {
    ...
    try {
      ...
      const alphaMate = -ShogiAIImprovedV18.MATE + ply;
      if (alpha < alphaMate) alpha = alphaMate;
      const betaMate = ShogiAIImprovedV18.MATE - ply;
      ...
      if (moves.length === 0) {
        ...
        return parentInCheck ? -ShogiAIImprovedV18.MATE + ply : 0;
      }
    ...
    let bestScore = -ShogiAIImprovedV18.INFINITE;
    ...
    for (let depth = 1; depth <= maxDepth; depth++) {
      try {
        ...
        const alpha0 = useAspiration ? bestScore - this.aspirationWindow : -ShogiAIImprovedV18.INFINITE;
        const beta0 = useAspiration ? bestScore + this.aspirationWindow : ShogiAIImprovedV18.INFINITE;
        ...
        if (useAspiration && (score <= alpha0 || score >= beta0)) {
          ...
          this.resetRootBest();
          score = this.search(position, depth, -ShogiAIImprovedV18.INFINITE, ShogiAIImprovedV18.INFINITE, 0);
        }
        ...
        if (bestScore >= ShogiAIImprovedV18.MATE - 10_000) break;
      } catch (e) {
    ...
    if (options.debug) {
      ...
      console.log(
        `[ShogiAIImprovedV18] depth=${completedDepth}/${maxDepth} score=${bestScore} nodes=${this.node} leaves=${this.leaf} time=${Math.round(elapsed)}ms`
      );
    }
...
const sharedAIV18 = new ShogiAIImprovedV18();
...
export function getBestMoveV18(k: KyokumenImproved, teban: number, difficulty: Difficulty, tesu: number = 0): Te | null {
  ...
  k.setTeban(teban);
  return sharedAIV18.getNextTe(k, tesu, { difficulty });
}
