 ...
 */
export class ShogiAIImprovedV19 {
  ...
  private killer1: number[] = new Array(ShogiAIImprovedV19.MAX_PLY).fill(0);
  private killer2: number[] = new Array(ShogiAIImprovedV19.MAX_PLY).fill(0);
  ...
  private prevKeyByPly: number[] = new Array(ShogiAIImprovedV19.MAX_PLY).fill(0);
  ...
  private moveLists: MoveListImproved[] = Array.from(
    { length: ShogiAIImprovedV19.MAX_PLY },
  ...
  private attackEpochByPly = new Int32Array(ShogiAIImprovedV19.MAX_PLY);
  private attackStampSente = new Int32Array(ShogiAIImprovedV19.MAX_PLY * ShogiAIImprovedV19.ATTACK_CACHE_SQUARES);
  private attackStampGote = new Int32Array(ShogiAIImprovedV19.MAX_PLY * ShogiAIImprovedV19.ATTACK_CACHE_SQUARES);
  private attackValSente = new Int32Array(ShogiAIImprovedV19.MAX_PLY * ShogiAIImprovedV19.ATTACK_CACHE_SQUARES);
  private attackValGote = new Int32Array(ShogiAIImprovedV19.MAX_PLY * ShogiAIImprovedV19.ATTACK_CACHE_SQUARES);
  ...
  constructor(tt: TranspositionTableImprovedPacked = new TranspositionTableImprovedPacked()) {
    ...
    this.evalCacheKeyV1 = new Int32Array(ShogiAIImprovedV19.EVAL_CACHE_SIZE);
    this.evalCacheValV1 = new Int32Array(ShogiAIImprovedV19.EVAL_CACHE_SIZE);
    this.evalCacheKeyV2 = new Int32Array(ShogiAIImprovedV19.EVAL_CACHE_SIZE);
    this.evalCacheValV2 = new Int32Array(ShogiAIImprovedV19.EVAL_CACHE_SIZE);
    this.evalCacheKeyV3 = new Int32Array(ShogiAIImprovedV19.EVAL_CACHE_SIZE);
    this.evalCacheValV3 = new Int32Array(ShogiAIImprovedV19.EVAL_CACHE_SIZE);

    this.evalCacheKeyV1.fill(ShogiAIImprovedV19.EVAL_CACHE_SENTINEL);
    this.evalCacheKeyV2.fill(ShogiAIImprovedV19.EVAL_CACHE_SENTINEL);
    this.evalCacheKeyV3.fill(ShogiAIImprovedV19.EVAL_CACHE_SENTINEL);
  }
  ...
  private evaluateSenteCached(k: KyokumenImproved): number {
    ...
    const index = key & (ShogiAIImprovedV19.EVAL_CACHE_SIZE - 1);
  ...
  private recordKiller(ply: number, key: number): void {
    ...
    if (ply < 0 || ply >= ShogiAIImprovedV19.MAX_PLY) return;
  ...
  private leastAttackerValueCached(k: KyokumenImproved, target: number, defender: number, ply: number): number {
    ...
    if ((stamp[index] | 0) === epoch) {
      ...
      return cached === ShogiAIImprovedV19.ATTACK_CACHE_INF ? Infinity : cached;
    }

    const computed = GenerateMovesImproved.getLeastAttackerValue(k, target, defender);
    const stored = Number.isFinite(computed) ? ((computed | 0) & 0x7fffffff) : ShogiAIImprovedV19.ATTACK_CACHE_INF;
    stamp[index] = epoch;
    val[index] = stored;
    return stored === ShogiAIImprovedV19.ATTACK_CACHE_INF ? Infinity : stored;
  }
  ...
  private scoreMove(k: KyokumenImproved, te: Te, ply: number, ttMoveKey: number, ttSecondMoveKey: number): number {
    ...
    if (ply > 0 && ply < ShogiAIImprovedV19.MAX_PLY) {
    ...
    // SEE-lite ordering for risky captures (cheap, all plies):
    ...
    if (te.from !== 0 && te.capture !== EMPTY) {
      ...
      if (pieceType === HI) score += 250_000;
	      ...
	      // Hanging-drop ordering (cheap, all plies):
  ...
  private quiescence(k: KyokumenImproved, alpha: number, beta: number, ply: number, depthLeft: number): number {
    ...
    try {
      ...
      if (allMoves.length === 0) {
        ...
        return inCheck ? -ShogiAIImprovedV19.MATE + ply : 0;
      }
  ...
  private search(k: KyokumenImproved, depthLeft: number, alpha: number, beta: number, ply: number): number {
    ...
    try {
      ...
      const alphaMate = -ShogiAIImprovedV19.MATE + ply;
      if (alpha < alphaMate) alpha = alphaMate;
      const betaMate = ShogiAIImprovedV19.MATE - ply;
      ...
      if (moves.length === 0) {
        ...
        return parentInCheck ? -ShogiAIImprovedV19.MATE + ply : 0;
      }
      ...
      const futilityApplicable =
        ...
        !parentInCheck &&
        depthLeft <= 2 &&
        alpha > -ShogiAIImprovedV19.MATE + 10_000 &&
        beta < ShogiAIImprovedV19.MATE - 10_000;
      ...
      for (const te of moves) {
        ...
        if (ply + 1 < ShogiAIImprovedV19.MAX_PLY) this.prevKeyByPly[ply + 1] = this.moveKey(te);
        ...
        if (searched === 0) {
          ...
          if (canLMR) {
            ...
            // V19: very late quiet moves at deeper nodes get one more ply of reduction.
            // Safe because any fail-high below is verified by a full-depth re-search.
            if (searched >= 12 && depthNext >= 4) reducedDepth = depthNext - 2;
          }
    ...
    let bestScore = -ShogiAIImprovedV19.INFINITE;
    ...
    for (let depth = 1; depth <= maxDepth; depth++) {
      try {
        ...
        const alpha0 = useAspiration ? bestScore - this.aspirationWindow : -ShogiAIImprovedV19.INFINITE;
        const beta0 = useAspiration ? bestScore + this.aspirationWindow : ShogiAIImprovedV19.INFINITE;
        ...
        if (useAspiration && (score <= alpha0 || score >= beta0)) {
          ...
          this.resetRootBest();
          score = this.search(position, depth, -ShogiAIImprovedV19.INFINITE, ShogiAIImprovedV19.INFINITE, 0);
        }
        ...
        if (bestScore >= ShogiAIImprovedV19.MATE - 10_000) break;
      } catch (e) {
    ...
    if (options.debug) {
      ...
      console.log(
        `[ShogiAIImprovedV19] depth=${completedDepth}/${maxDepth} score=${bestScore} nodes=${this.node} leaves=${this.leaf} time=${Math.round(elapsed)}ms`
      );
    }
...
const sharedAIV19 = new ShogiAIImprovedV19();

/**
 * Exported helper for UI/script compatibility.
 * Used by `/games/shogi-improved` and the fast-search path in `/games/shogi`.
 */
export function getBestMoveV19(k: KyokumenImproved, teban: number, difficulty: Difficulty, tesu: number = 0): Te | null {
  // The UI passes `teban` explicitly; keep the position consistent.
  k.setTeban(teban);
  return sharedAIV19.getNextTe(k, tesu, { difficulty });
}
