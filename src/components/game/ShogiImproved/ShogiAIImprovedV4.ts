 ...
 * - Draw "contempt" on repetition nodes (discourage draws when ahead).
 * - Null-move pruning (Expert/Master only) to search deeper under the same time budget.
 * - Limited check extensions (Expert/Master only) to improve tactical accuracy.
 * - Quiescence delta pruning to reduce noisy-branch explosion.
 * - Always returns a legal move at the root (fallback), even under very tight time budgets.
 *
 ...
 */
export class ShogiAIImprovedV4 {
  ...
  private killer1: number[] = new Array(ShogiAIImprovedV4.MAX_PLY).fill(0);
  private killer2: number[] = new Array(ShogiAIImprovedV4.MAX_PLY).fill(0);
  ...
  private recordKiller(ply: number, key: number): void {
    ...
    if (ply < 0 || ply >= ShogiAIImprovedV4.MAX_PLY) return;
  ...
  private quiescence(k: KyokumenImproved, alpha: number, beta: number, ply: number, depthLeft: number): number {
    ...
    try {
      ...
      if (allMoves.length === 0) {
        ...
        return inCheck ? -ShogiAIImprovedV4.MATE + ply : 0;
      }
      ...
      for (const te of moves) {
        ...
        if (!inCheck && this.enableDeltaPruning) {
  ...
  private search(k: KyokumenImproved, depthLeft: number, alpha: number, beta: number, ply: number): number {
    ...
    try {
      ...
      if (moves.length === 0) {
        ...
        return parentInCheck ? -ShogiAIImprovedV4.MATE + ply : 0;
      }
  ...
  getNextTe(k: KyokumenImproved, tesu: number = 0, options: ShogiAISearchOptions = {}): Te | null {
    ...
    // V4 tuning notes:
    // - Check extensions can help tactics but also distort search if applied everywhere.
    //   We keep them shallow (near root) and only on Master for now.
    // - Delta pruning and draw contempt are intentionally disabled by default because they can be risky
    //   (tactical misses / unnecessary "play for win" blunders). They can be re-enabled once tuned via self-play.
    ...
    let bestScore = -ShogiAIImprovedV4.INFINITE;
    ...
    for (let depth = 1; depth <= maxDepth; depth++) {
      try {
        ...
        const alpha0 = useAspiration ? bestScore - this.aspirationWindow : -ShogiAIImprovedV4.INFINITE;
        const beta0 = useAspiration ? bestScore + this.aspirationWindow : ShogiAIImprovedV4.INFINITE;
        ...
        if (useAspiration && (score <= alpha0 || score >= beta0)) {
          ...
          this.resetRootBest();
          score = this.search(position, depth, -ShogiAIImprovedV4.INFINITE, ShogiAIImprovedV4.INFINITE, 0);
        }
        ...
        if (bestScore >= ShogiAIImprovedV4.MATE - 10_000) break;
      } catch (e) {
    ...
    if (options.debug) {
      ...
      console.log(
        `[ShogiAIImprovedV4] depth=${completedDepth}/${maxDepth} score=${bestScore} nodes=${this.node} leaves=${this.leaf} time=${Math.round(elapsed)}ms`
      );
    }
...
const sharedAIV4 = new ShogiAIImprovedV4();
 ...
 */
export function getBestMoveV4(k: KyokumenImproved, teban: number, difficulty: Difficulty): Te | null {
  // The UI passes `teban` explicitly; keep the position consistent.
  k.setTeban(teban);
  return sharedAIV4.getNextTe(k, 0, { difficulty });
}
