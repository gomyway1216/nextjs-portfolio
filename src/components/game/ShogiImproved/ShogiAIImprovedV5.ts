 ...
 */
export class ShogiAIImprovedV5 {
  ...
  private killer1: number[] = new Array(ShogiAIImprovedV5.MAX_PLY).fill(0);
  private killer2: number[] = new Array(ShogiAIImprovedV5.MAX_PLY).fill(0);
  ...
  private recordKiller(ply: number, key: number): void {
    ...
    if (ply < 0 || ply >= ShogiAIImprovedV5.MAX_PLY) return;
  ...
  private quiescence(k: KyokumenImproved, alpha: number, beta: number, ply: number, depthLeft: number): number {
    ...
    try {
      ...
      if (allMoves.length === 0) {
        ...
        return inCheck ? -ShogiAIImprovedV5.MATE + ply : 0;
      }
  ...
  private search(k: KyokumenImproved, depthLeft: number, alpha: number, beta: number, ply: number): number {
    ...
    try {
      ...
      const alphaMate = -ShogiAIImprovedV5.MATE + ply;
      if (alpha < alphaMate) alpha = alphaMate;
      const betaMate = ShogiAIImprovedV5.MATE - ply;
      ...
      if (moves.length === 0) {
        ...
        return parentInCheck ? -ShogiAIImprovedV5.MATE + ply : 0;
      }
  ...
  getNextTe(k: KyokumenImproved, tesu: number = 0, options: ShogiAISearchOptions = {}): Te | null {
    ...
    let bestScore = -ShogiAIImprovedV5.INFINITE;
    ...
    for (let depth = 1; depth <= maxDepth; depth++) {
      try {
        ...
        const alpha0 = useAspiration ? bestScore - this.aspirationWindow : -ShogiAIImprovedV5.INFINITE;
        const beta0 = useAspiration ? bestScore + this.aspirationWindow : ShogiAIImprovedV5.INFINITE;
        ...
        if (useAspiration && (score <= alpha0 || score >= beta0)) {
          ...
          this.resetRootBest();
          score = this.search(position, depth, -ShogiAIImprovedV5.INFINITE, ShogiAIImprovedV5.INFINITE, 0);
        }
        ...
        if (bestScore >= ShogiAIImprovedV5.MATE - 10_000) break;
      } catch (e) {
    ...
    if (options.debug) {
      ...
      console.log(
        `[ShogiAIImprovedV5] depth=${completedDepth}/${maxDepth} score=${bestScore} nodes=${this.node} leaves=${this.leaf} time=${Math.round(elapsed)}ms`
      );
    }
...
const sharedAIV5 = new ShogiAIImprovedV5();
 ...
 */
export function getBestMoveV5(k: KyokumenImproved, teban: number, difficulty: Difficulty): Te | null {
  // The UI passes `teban` explicitly; keep the position consistent.
  k.setTeban(teban);
  return sharedAIV5.getNextTe(k, 0, { difficulty });
}
