 ...
 */
export class ShogiAIImprovedV8 {
  ...
  private killer1: number[] = new Array(ShogiAIImprovedV8.MAX_PLY).fill(0);
  private killer2: number[] = new Array(ShogiAIImprovedV8.MAX_PLY).fill(0);
  ...
  constructor(tt: TranspositionTableImproved = new TranspositionTableImproved()) {
    ...
    this.evalCacheKeyV1 = new Int32Array(ShogiAIImprovedV8.EVAL_CACHE_SIZE);
    this.evalCacheValV1 = new Int32Array(ShogiAIImprovedV8.EVAL_CACHE_SIZE);
    this.evalCacheKeyV2 = new Int32Array(ShogiAIImprovedV8.EVAL_CACHE_SIZE);
    this.evalCacheValV2 = new Int32Array(ShogiAIImprovedV8.EVAL_CACHE_SIZE);
    this.evalCacheKeyV3 = new Int32Array(ShogiAIImprovedV8.EVAL_CACHE_SIZE);
    this.evalCacheValV3 = new Int32Array(ShogiAIImprovedV8.EVAL_CACHE_SIZE);

    this.evalCacheKeyV1.fill(ShogiAIImprovedV8.EVAL_CACHE_SENTINEL);
    this.evalCacheKeyV2.fill(ShogiAIImprovedV8.EVAL_CACHE_SENTINEL);
    this.evalCacheKeyV3.fill(ShogiAIImprovedV8.EVAL_CACHE_SENTINEL);
  }
  ...
  private evaluateSenteCached(k: KyokumenImproved): number {
    ...
    const index = key & (ShogiAIImprovedV8.EVAL_CACHE_SIZE - 1);
  ...
  private recordKiller(ply: number, key: number): void {
    ...
    if (ply < 0 || ply >= ShogiAIImprovedV8.MAX_PLY) return;
  ...
  private scoreMove(k: KyokumenImproved, te: Te, ply: number, ttMoveKey: number, ttSecondMoveKey: number): number {
    ...
    if (te.from === 0) {
      ...
      // Root-only drop safety:
      // If a drop is immediately capturable and not defended, it's often a wasted tempo (or an outright blunder)
      // at the shallow depths we can afford in the UI. Penalize such drops in move ordering.
      if (ply === 0) {
        score += this.rootDropSafetyOrderAdjustment(k, te, enemyKing);
      }
    } else {
  ...
  private rootDropSafetyOrderAdjustment(k: KyokumenImproved, te: Te, enemyKing: number): number {
    if (te.from !== 0) return 0;

    // If it's a very close-to-king drop, it may be a checking/tactical drop; don't discourage it.
    if (enemyKing > 0) {
      const dist =
        Math.abs((te.to >> 4) - (enemyKing >> 4)) + Math.abs((te.to & 0x0f) - (enemyKing & 0x0f));
      if (dist <= 2) return 0;
    ...
    const enemyCanCapture = GenerateMovesImproved.isSquareAttacked(k, te.to, k.teban);
    if (!enemyCanCapture) return 0;

    const selfDefends = GenerateMovesImproved.isSquareAttacked(k, te.to, this.otherSide(k.teban));
    if (selfDefends) return 25_000;
  ...
  private quiescence(k: KyokumenImproved, alpha: number, beta: number, ply: number, depthLeft: number): number {
    ...
    try {
      ...
      if (allMoves.length === 0) {
        ...
        return inCheck ? -ShogiAIImprovedV8.MATE + ply : 0;
      }
  ...
  private search(k: KyokumenImproved, depthLeft: number, alpha: number, beta: number, ply: number): number {
    ...
    try {
      ...
      const alphaMate = -ShogiAIImprovedV8.MATE + ply;
      if (alpha < alphaMate) alpha = alphaMate;
      const betaMate = ShogiAIImprovedV8.MATE - ply;
      ...
      if (moves.length === 0) {
        ...
        return parentInCheck ? -ShogiAIImprovedV8.MATE + ply : 0;
      }
  ...
  getNextTe(k: KyokumenImproved, tesu: number = 0, options: ShogiAISearchOptions = {}): Te | null {
    ...
    let bestScore = -ShogiAIImprovedV8.INFINITE;
    ...
    for (let depth = 1; depth <= maxDepth; depth++) {
      try {
        ...
        const alpha0 = useAspiration ? bestScore - this.aspirationWindow : -ShogiAIImprovedV8.INFINITE;
        const beta0 = useAspiration ? bestScore + this.aspirationWindow : ShogiAIImprovedV8.INFINITE;
        ...
        if (useAspiration && (score <= alpha0 || score >= beta0)) {
          ...
          this.resetRootBest();
          score = this.search(position, depth, -ShogiAIImprovedV8.INFINITE, ShogiAIImprovedV8.INFINITE, 0);
        }
        ...
        if (bestScore >= ShogiAIImprovedV8.MATE - 10_000) break;
      } catch (e) {
    ...
    if (options.debug) {
      ...
      console.log(
        `[ShogiAIImprovedV8] depth=${completedDepth}/${maxDepth} score=${bestScore} nodes=${this.node} leaves=${this.leaf} time=${Math.round(elapsed)}ms`
      );
    }
...
const sharedAIV8 = new ShogiAIImprovedV8();
 ...
 */
export function getBestMoveV8(k: KyokumenImproved, teban: number, difficulty: Difficulty): Te | null {
  // The UI passes `teban` explicitly; keep the position consistent.
  k.setTeban(teban);
  return sharedAIV8.getNextTe(k, 0, { difficulty });
}
