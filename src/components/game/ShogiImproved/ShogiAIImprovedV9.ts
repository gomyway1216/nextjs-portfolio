 ...
 *
 * V9 additions:
 * - Opening-aware root move ordering (when book doesn't apply) for more human-like openings.
 * - Root move safety ordering (SEE-lite / hanging detection) to reduce obviously bad drops and loose moves.
 ...
 */
export class ShogiAIImprovedV9 {
  ...
  private killer1: number[] = new Array(ShogiAIImprovedV9.MAX_PLY).fill(0);
  private killer2: number[] = new Array(ShogiAIImprovedV9.MAX_PLY).fill(0);
  ...
  constructor(tt: TranspositionTableImproved = new TranspositionTableImproved()) {
    ...
    this.evalCacheKeyV1 = new Int32Array(ShogiAIImprovedV9.EVAL_CACHE_SIZE);
    this.evalCacheValV1 = new Int32Array(ShogiAIImprovedV9.EVAL_CACHE_SIZE);
    this.evalCacheKeyV2 = new Int32Array(ShogiAIImprovedV9.EVAL_CACHE_SIZE);
    this.evalCacheValV2 = new Int32Array(ShogiAIImprovedV9.EVAL_CACHE_SIZE);
    this.evalCacheKeyV3 = new Int32Array(ShogiAIImprovedV9.EVAL_CACHE_SIZE);
    this.evalCacheValV3 = new Int32Array(ShogiAIImprovedV9.EVAL_CACHE_SIZE);

    this.evalCacheKeyV1.fill(ShogiAIImprovedV9.EVAL_CACHE_SENTINEL);
    this.evalCacheKeyV2.fill(ShogiAIImprovedV9.EVAL_CACHE_SENTINEL);
    this.evalCacheKeyV3.fill(ShogiAIImprovedV9.EVAL_CACHE_SENTINEL);
  }
  ...
  private evaluateSenteCached(k: KyokumenImproved): number {
    ...
    const index = key & (ShogiAIImprovedV9.EVAL_CACHE_SIZE - 1);
  ...
  private recordKiller(ply: number, key: number): void {
    ...
    if (ply < 0 || ply >= ShogiAIImprovedV9.MAX_PLY) return;
  ...
  private quiescence(k: KyokumenImproved, alpha: number, beta: number, ply: number, depthLeft: number): number {
    ...
    try {
      ...
      if (allMoves.length === 0) {
        ...
        return inCheck ? -ShogiAIImprovedV9.MATE + ply : 0;
      }
  ...
  private search(k: KyokumenImproved, depthLeft: number, alpha: number, beta: number, ply: number): number {
    ...
    try {
      ...
      const alphaMate = -ShogiAIImprovedV9.MATE + ply;
      if (alpha < alphaMate) alpha = alphaMate;
      const betaMate = ShogiAIImprovedV9.MATE - ply;
      ...
      if (moves.length === 0) {
        ...
        return parentInCheck ? -ShogiAIImprovedV9.MATE + ply : 0;
      }
  ...
  getNextTe(k: KyokumenImproved, tesu: number = 0, options: ShogiAISearchOptions = {}): Te | null {
    ...
    let bestScore = -ShogiAIImprovedV9.INFINITE;
    ...
    for (let depth = 1; depth <= maxDepth; depth++) {
      try {
        ...
        const alpha0 = useAspiration ? bestScore - this.aspirationWindow : -ShogiAIImprovedV9.INFINITE;
        const beta0 = useAspiration ? bestScore + this.aspirationWindow : ShogiAIImprovedV9.INFINITE;
        ...
        if (useAspiration && (score <= alpha0 || score >= beta0)) {
          ...
          this.resetRootBest();
          score = this.search(position, depth, -ShogiAIImprovedV9.INFINITE, ShogiAIImprovedV9.INFINITE, 0);
        }
        ...
        if (bestScore >= ShogiAIImprovedV9.MATE - 10_000) break;
      } catch (e) {
    ...
    if (options.debug) {
      ...
      console.log(
        `[ShogiAIImprovedV9] depth=${completedDepth}/${maxDepth} score=${bestScore} nodes=${this.node} leaves=${this.leaf} time=${Math.round(elapsed)}ms`
      );
    }
...
const sharedAIV9 = new ShogiAIImprovedV9();
 ...
 */
export function getBestMoveV9(k: KyokumenImproved, teban: number, difficulty: Difficulty): Te | null {
  // The UI passes `teban` explicitly; keep the position consistent.
  k.setTeban(teban);
  return sharedAIV9.getNextTe(k, 0, { difficulty });
}
