...
// so every handicap starts with teban = GOTE.
...
// (SENTE first); every handicap flips the first move to the 上手 (GOTE).
...
const ShogiImproved = () => {
  ...
  // Snapshots of the position *before* each of the player's moves, newest last.
  // 待った (takeback) restores the latest one, undoing the player's last move
  // together with the AI's reply. Only the player's turns are recorded because
  // that is the only state a takeback ever returns to.
  const [history, setHistory] = useState<{ kyokumen: KyokumenImproved; ply: number }[]>([]);
  ...
  // Execute move
	  const executeMove = (te: Te, promote: boolean) => {
	    // Record the pre-move position so 待った can return here. gameState.kyokumen
	    // is never mutated (executeMove clones before moving), so it is a safe
	    // immutable snapshot.
    ...
    setHistory(h => [...h, { kyokumen: prevKyokumen, ply: prevPly }]);
  ...
  // player's move (never mid-AI-search or after game over).
  const canUndo =
              ...
              <div style={{ width: '100%' }}>
                ...
                {handicap !== 'none' && (
                  <p style={{ marginTop: '0.6rem', fontSize: '0.78rem', lineHeight: 1.5, opacity: 0.7 }}>
                    駒落ちではAI（上手）が先に指します。AIの評価関数は平手で学習しているため、
                    駒落ちでの強さは平手と同一ではありません。
                  </p>
                )}
              </div>
        ...
        additionalContent={
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            <button
              onClick={handleUndo}
              disabled={!canUndo}
              title={canUndo ? '待った（1手戻す）' : '自分の手番でのみ使えます'}
              style={{
                padding: '6px 14px',
                borderRadius: '6px',
