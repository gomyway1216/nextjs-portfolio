 ...
 *
 * Entry format (8 x i32 = 32 bytes, index = hash & MASK):
 *   [0] check  = hash ^ value ^ flagDepth ^ best ^ second
 *   [1] value
 ...
 * 32-bit hash guarantee as the single-thread table, so a rare false hit is no
 * worse than today's hash collisions.
 *
...
export class SharedTT {
  ...
  /**
   * Probe for `hash`. On a hit writes [value, flagDepth, best, second] into
   * `scratch` (a view into the calling instance's memory) and returns 1.
   */
  probe(hash: number, scratch: Int32Array): number {
    const view = this.view;
    const base = HEADER_I32 + ((hash & MASK) << 3);
    ...
    if ((check ^ value ^ flagDepth ^ best ^ second) !== hash) return 0; // other position or torn
    scratch[0] = value;
    scratch[1] = flagDepth;
    scratch[2] = best;
    scratch[3] = second;
  ...
  /**
   * Store an entry. Ports the engine's replacement policy: same position is
   * only replaced by an equal-or-deeper search (the old best move becomes the
   * `second` ordering hint); a different position always replaces.
   */
  store(hash: number, value: number, flagDepth: number, best: number): void {
    const view = this.view;
    const base = HEADER_I32 + ((hash & MASK) << 3);
    ...
    let second = 0;
    if (oFlagDepth !== 0 && (oCheck ^ oValue ^ oFlagDepth ^ oBest ^ oSecond) === hash) {
    ...
    Atomics.store(view, base, hash ^ value ^ flagDepth ^ best ^ second);
  }
