...
function OwnerLearningLibrary() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const result = await call('search', { query, view, offset, limit: 20, ...(domain !== 'all' ? { domain } : {}), ...(kind !== 'all' ? { kind } : {}) });
  ...
  }, [call, currentUser, isAdmin, query, domain, kind, view, offset]);
  ...
  async function save(event: FormEvent) {
    ...
    try {
      ...
      if (query || domain !== 'all' || kind !== 'all' || view !== 'all' || offset) {
        setBusy(true); setQuery(''); setDomain('all'); setKind('all'); setView('all'); setOffset(0);
      } else { await load(); }
  ...
  async function openRelated(id: string) {
    setError('');
    try { const result = await call('search', { id }); setItems((previous) => [result.item, ...previous.filter((item) => item.id !== id)]); setExpanded(id); }
    catch (failure) { setError(failure instanceof Error ? failure.message : 'Unable to load'); }
  }
  ...
  return <main className="mx-auto max-w-6xl space-y-6 px-4 py-8 sm:px-8">
    ...
    <section className="space-y-4 rounded-xl border bg-card p-4">
      <Tabs value={view} onValueChange={(value) => { setView(value); setOffset(0); }}><TabsList><TabsTrigger value="all">{say('すべての学び', 'All learnings')}</TabsTrigger><TabsTrigger value="review">{say('少し復習する', 'A little review')}</TabsTrigger></TabsList></Tabs>
      <div className="flex flex-wrap gap-3">
        <div className="relative min-w-48 flex-1"><Search size={18} className="absolute left-3 top-3 text-muted-foreground" /><Input aria-label={say('学びを検索', 'Search learnings')} className="pl-10" placeholder={say('単語・コンセプト・メモから検索', 'Search words, concepts and notes')} value={query} onChange={(e) => { setQuery(e.target.value); setOffset(0); }} /></div>
        <Select value={domain} onValueChange={(v) => { setDomain(v); setOffset(0); }}><SelectTrigger className="w-full sm:w-52" aria-label={say('分野', 'Domain')}><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">{say('すべての分野', 'All domains')}</SelectItem>{LEARNING_DOMAINS.map((d) => <SelectItem key={d} value={d}>{labels.domains[d]}</SelectItem>)}</SelectContent></Select>
        <Select value={kind} onValueChange={(v) => { setKind(v); setOffset(0); }}><SelectTrigger className="w-full sm:w-44" aria-label={say('種類', 'Kind')}><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">{say('すべての種類', 'All kinds')}</SelectItem>{LEARNING_KINDS.map((k) => <SelectItem key={k} value={k}>{labels.kinds[k]}</SelectItem>)}</SelectContent></Select>
      </div>
