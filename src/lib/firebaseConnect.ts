...
import { firebaseClientConfig, isFirebaseClientConfigured } from './firebaseConfig';
...
export const signInWithSessionCookie = async (onBeforeSignIn?: (uid: string, isAdmin?: boolean) => void) => {
  if (!app) return null;
  ...
  const data = await response.json() as { customToken?: unknown; uid?: unknown; isAdmin?: unknown };
  ...
  if (typeof data.uid === 'string') {
    onBeforeSignIn?.(data.uid, typeof data.isAdmin === 'boolean' ? data.isAdmin : undefined);
  }
