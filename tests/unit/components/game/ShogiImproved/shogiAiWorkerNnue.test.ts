 ...
 * worker fetches /shogi-halfkp81-production-weights.bin at startup), then the difficulty
...
beforeAll(async () => {
  const weights = readFileSync(join(process.cwd(), 'public', 'shogi-halfkp81-production-weights.bin'));
  vi.stubGlobal(
    'fetch',
    vi.fn(async (url: unknown) => {
      ...
      if (String(url) === '/shogi-opening-book-v3.bin') return { ok: false, status: 404 };
      expect(String(url)).toBe('/shogi-halfkp81-production-weights.bin');
      return {
        ok: true,
