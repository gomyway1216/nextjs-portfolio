...
describe('wasmEngine NNUE loading', () => {
  ...
  it(
    ...
    () => {
      ...
      // One trial is enough because this is DETERMINISTIC — see MOVE_DEPTH.
      const TRIALS = 1;
      const AI_MOVES = 20;
      // A FIXED depth, with the clock only as a runaway guard.
      //
      // A wall-clock budget makes the outcome depend on how much CPU the
      // search actually gets, and this file shares a runner with everything
      // else: at 300ms/move the same engine shuttled 0-1 times on an idle
      // laptop and 3-4 times on a loaded CI box, so any threshold was either
      // vacuous or flaky. At a fixed depth the engine searches the same tree
      // everywhere and the counts below are exact and reproducible.
      //
      // Depth 6 specifically: it costs ~1.3s for the whole 20-move
      // continuation here, which leaves an enormous margin under MOVE_CAP_MS
      // on a slower runner — the cap must never actually bite, or the result
      // becomes time-dependent again. (The version this replaced ran eight
      // UNCAPPED depth-11 searches and once took ~350s on CI, so the cap
      // stays as a backstop.)
      const MOVE_DEPTH = 6;
      const MOVE_CAP_MS = 8_000;
      const trials: { pawnDrops8f: number; maxAiSideRepeat: number }[] = [];
      ...
      for (let trial = 0; trial < TRIALS; trial++) {
        ...
        trials.push({
          pawnDrops8f,
          maxAiSideRepeat: Math.max(...aiSideCounts.values()),
        });
      }

      // Current, reproducible behaviour: the engine spends the pawn twice and
      // then leaves the cycle. The reported game was FOUR complete cycles in
      // twenty-two plies with no way out, so these bounds sit just above what
      // the engine actually does and well below the bug — a regression that
      // puts it back in the shuttle moves these numbers up, not sideways.
      //
      // Note what this test does NOT claim: it does not distinguish the
      // pre-fix runtime from the fixed one (the loop rate is not measurably
      // different between them). The mechanisms this change fixes are pinned
      // deterministically in searchRepetitionFix.test.ts. This test's job is
      // to keep the realistic game SHAPE — scripted cooperating Sente, engine
      // on Gote only, warm TT — under continuous exercise.
      const report = trials
        .map((t, i) => `trial${i + 1}: P*8f=${t.pawnDrops8f} aiRepeat=${t.maxAiSideRepeat}`)
        .join(' | ');
      expect(Math.max(...trials.map((t) => t.pawnDrops8f)), report).toBeLessThanOrEqual(2);
      // And the AI's own setup must not keep coming back either, which is
      // what the shuttle looks like once the opponent varies.
      expect(Math.max(...trials.map((t) => t.maxAiSideRepeat)), report).toBeLessThanOrEqual(2);
    },
    // ~1.5s in practice; the ceiling is AI_MOVES * MOVE_CAP_MS even if the
    // evaluator changes underneath it.
    180_000,
  );
