...
describe("Floodgate v7 portable copy witness foundation evidence", () => {
  it("pins the dormant filesystem-only boundary and exact state transition", () => {
    ...
    expect(record).toMatchObject({
      ...
      source_base: {
        earlier_origin_main_integrated:
        ...
        latest_main_pull_request: 518,
        integration_method: "regular-merge-commit",
        integration_commit: "df7118cd81aefa932f033399a96475ae6069d11b",
        integration_tree: "3687786f3a7658b418b436b769c92a2c88b3942e",
        integration_parents: [
          "70a7dd89584ba5c21c9c8720561f3c9794558ef4",
          "3bdf6d1127b86401ef08854737c700629a2d2ea7",
        ],
        latest_main_contains_failure_kind_intrinsic_hardening: true,
        latest_main_contains_local_checkpoint_runtime_claim_order_fix: true,
        readme_preserves_portable_foundation_and_checkpoint_runtime_claim_sections: true,
      },
      implementation: {
        ...
        validated_source_revision: "df7118cd81aefa932f033399a96475ae6069d11b",
        ...
        ci_failure_and_test_isolation_hardening: {
          initial_pushed_head: "bfcf9773ae68f00ba29dab1303a379050fdd1e67",
          initial_pushed_head_core_ci: {
            workflow_run_id: 29684249170,
            job_id: 88185671269,
            result: "PASS",
          },
          triggering_head: "b818f9a47e64c64fb26711c7226b4aa2b92d2b88",
          triggering_head_role:
            "first-post-intrinsic-hardening-follow-up-head-that-triggered-the-same-realm-test-isolation-failure-not-the-first-pushed-pr-head",
          ...
          triggering_ci_tests: {
            ...
            failed: 1,
            skipped: 150,
            unreported_or_unclassified: 4,
            total: 3344,
          },
          triggering_ci_test_accounting_explanation:
            "the raw Vitest footer reported total 3344 while passed+failed+skipped covered only 3340; four tasks were unreported or unclassified after the same-realm Array.prototype.includes pollution disrupted Vitest task accounting",
        ...
        subsequent_unrelated_ci_failure_after_isolation: {
          head: "70a7dd89584ba5c21c9c8720561f3c9794558ef4",
          workflow_run_id: 29685967727,
          core_job_id: 88190142230,
          failed_file:
            "tests/unit/ml/floodgateStableProposalFinalizationResume.test.ts",
          failed_test: "does not steal a stale authorization marker",
          expected_retry_disposition:
            "resume-finalization-over-complete-authenticated-work",
          received_retry_disposition:
          ...
          wall_seconds: 94.16,
          portable_copy_witness_test_failed: false,
          failed_file_changed_by_pull_request_517: false,
          same_target_node_22_13_repetitions: {
            passed: 10,
            failed: 0,
          },
          parallel_pull_request_518_core_ci: {
            head: "fe85cbaa2a2553853b7bab75a4e563408599a6f5",
            workflow_run_id: 29685939467,
            job_id: 88190069831,
            result: "PASS",
          },
          classification:
            "unrelated-existing-test-nondeterminism-not-reproduced-in-ten-exact-target-runs-or-the-parallel-main-bound-core-run",
        },
      },
      filesystem_protocol: {
        fixed_kinds: [
    ...
    const implementationRecord = record.implementation as {
      ci_failure_and_test_isolation_hardening: {
        triggering_ci_tests: {
          passed: number;
          failed: number;
          skipped: number;
          unreported_or_unclassified: number;
          total: number;
        };
      };
      subsequent_unrelated_ci_failure_after_isolation: {
    ...
    const hardening =
      implementationRecord.ci_failure_and_test_isolation_hardening;
    const triggeringCounts = hardening.triggering_ci_tests;
    expect(
      triggeringCounts.passed +
        triggeringCounts.failed +
        triggeringCounts.skipped +
        triggeringCounts.unreported_or_unclassified,
    ).toBe(triggeringCounts.total);

    const subsequent =
      implementationRecord.subsequent_unrelated_ci_failure_after_isolation;
    expect(
      subsequent.tests.passed +
        subsequent.tests.failed +
        subsequent.tests.skipped,
    ).toBe(subsequent.tests.total);
    const previousMain = (
      record.source_base as {
        previous_origin_main_integrated: string;
      }
    ).previous_origin_main_integrated;
    expect(
  ...
  it("binds implementation commits, the regular main merge, and exact file identities", () => {
    ...
    const sourceBase = record.source_base as {
      ...
      readme_preserves_portable_foundation_and_checkpoint_runtime_claim_sections: boolean;
    ...
    expect(
      sourceBase.readme_preserves_portable_foundation_and_checkpoint_runtime_claim_sections,
    ).toBe(true);
    expect(
  ...
  it("records local-only validation, zero runtime execution, and bilingual disclosure", () => {
    ...
    expect(record.local_validation).toMatchObject({
      ...
      expanded_runner_regression: {
        ...
        failed: 0,
        wall_seconds: 1.73,
        validated_before_latest_main_integration: true,
      },
      post_latest_main_integration: {
        revision: "df7118cd81aefa932f033399a96475ae6069d11b",
        node: "v22.13.0",
        expanded_runner_regression: {
          test_files: 7,
          passed: 113,
        ...
        unrelated_stale_authorization_marker_exact_target_repetitions: {
          passed: 10,
          failed: 0,
        },
      },
