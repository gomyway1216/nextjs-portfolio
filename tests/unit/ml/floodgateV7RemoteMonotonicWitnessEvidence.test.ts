  ...
  it("pins the reviewed implementation commit while every production counter stays zero", () => {
    ...
    expect(evidence).toMatchObject({
      ...
      revision: {
        ...
        publication_revision: "320722870b67190c42b48838b8d17feb99d1faa3",
        publication_tree: "c4a9ba9facfd818db76a02a2ef6e14f2adb6316d",
    ...
    for (const relativePath of expectedArticlePaths) {
      ...
      const anchoredBlob = gitOutput([
        "--no-replace-objects",
        "rev-parse",
        `${evidence.revision.publication_revision}:${relativePath}`,
      ]);
      expect(currentBlob, relativePath).toBe(anchoredBlob);
    }
  });
