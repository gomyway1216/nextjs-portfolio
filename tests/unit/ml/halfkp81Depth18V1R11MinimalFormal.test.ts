  ...
  it("accepts every output sealed by the tracked v1r11 namespace", () => {
    const trackedPlan = JSON.parse(
      fs.readFileSync(
        path.join(
          ROOT,
          "ml/halfkp81-hard-depth18-yaneura-only-v1r11-minimal-r3-plan.json",
        ),
        "utf8",
      ),
  ...
  it("binds the exact minimal entrypoint identity in both tracked contracts", () => {
    ...
    const preregistration = JSON.parse(
      fs.readFileSync(
        path.join(
          ROOT,
          "docs/data/shogi-halfkp81-depth18-yaneura-only-v1r11-minimal-r3-preregistration-2026-08-02.json",
        ),
        "utf8",
      ),
    ) as Record<string, Record<string, unknown>>;
