...
describe("SFEN and USI bridge for sibling positions", () => {
  ...
  it("resolves promotion and drops only through the legal move list", () => {
    ...
    expect(
      teToUsi(resolveUsiMove(positionFromSfen(promotion).position, "7g2b+")),
    ).toBe("7g2b+");
    expect(childSfenAfterUsi(promotion, "7g2b+")).toContain(
      "7+B1/9/9/9/9/9/9/4K4 w - 2",
    );

    const drop = "4k4/9/9/9/9/9/9/9/4K4 b P 1";
    expect(
      teToUsi(resolveUsiMove(positionFromSfen(drop).position, "P*5e")),
    ).toBe("P*5e");
    expect(childSfenAfterUsi(drop, "P*5e")).toBe(
      "4k4/9/9/9/4P4/9/9/9/4K4 w - 2",
    );
  });
  ...
  it("restores optional bishop/rook non-promotion in one deterministic rules-complete set", () => {
    ...
    for (const fixture of cases) {
      ...
      const second = rulesCompleteLegalMoves(position).map(
        (entry) => entry.usi,
      );
      ...
      expect(teToUsi(resolveUsiMove(position, fixture.declined))).toBe(
        fixture.declined,
      );
    }
  });

  it("keeps direct USI string order equivalent to UTF-8 byte order", () => {
    const positions = [
      START,
      "4k4/9/9/9/4B4/9/9/9/K8 b - 1",
      "4k4/9/9/9/9/9/9/9/4K4 b PLNSGBR 1",
    ];
    for (const sfen of positions) {
      const moves = rulesCompleteLegalMoves(
        positionFromSfen(sfen).position,
      ).map((entry) => entry.usi);
      expect(moves.every((usi) => /^[\x20-\x7e]+$/.test(usi))).toBe(true);
  ...
  it("distinguishes a one-reply parent from schema-eligible parents with at least two children", () => {
    ...
    expect(
      rulesCompleteLegalMoves(positionFromSfen(checkmate).position),
    ).toHaveLength(0);
    expect(
      rulesCompleteLegalMoves(positionFromSfen(oneReply).position).map(
        (entry) => entry.usi,
      ),
    ).toEqual(["5a4a"]);
    expect(
  ...
  it("rejects malformed SFEN and illegal USI instead of guessing", () => {
    ...
    expect(() =>
      resolveUsiMove(positionFromSfen(START).position, "7g7e"),
    ).toThrow(/0 legal moves/);
    expect(() =>
      resolveUsiMove(positionFromSfen(START).position, "P*5e"),
    ).toThrow(/0 legal moves/);
  });
});
