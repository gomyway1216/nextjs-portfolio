...
describe("USI teacher engine subprocess contract", () => {
  ...
  it("initializes and returns an exact fixed-depth candidate snapshot", async () => {
    ...
    try {
      ...
      expect(
        result.lines.map((line) => [line.multipv, line.move, line.cp]),
      ).toEqual([
        [1, "7g7f", 260],
        [2, "2g2f", 220],
      ]);
  ...
  it("rejects a parent reset deadline with a distinct typed timeout signal", async () => {
    const engine = new UsiTeacherEngine({
      ...
      engineArgs: [FAKE_ENGINE],
      timeoutMs: 5_000,
      testOnlyResetForParentTimeoutMs: 25,
    });
    try {
      await engine.init();
      ...
      const send = internal.send.bind(engine);
      vi.spyOn(internal, "send").mockImplementation((command) => {
        if (command !== "isready") send(command);
      });
      const failure = await engine
        .resetForParent()
        .catch((error: unknown) => error);
      expect(failure).toBeInstanceOf(UsiResetForParentTimeoutError);
      expect(failure).toMatchObject({
        name: "UsiResetForParentTimeoutError",
        timeoutMs: 25,
        message: "USI resetForParent timeout after 25ms",
      });
    } finally {
      await engine.quit();
    }
  ...
  it("kills and closes a live child whose stdin pipe fails", async () => {
    ...
    try {
      ...
      await expect(initialization).rejects.toThrow(
        /USI stdin error: synthetic EPIPE/,
      );
