...
describe('USI MultiPV parsing', () => {
  it('parses exact cp and mate PV records and excludes bounds', () => {
    expect(
      parseUsiInfoLine('info depth 8 seldepth 12 multipv 2 score cp -42 nodes 901 pv 3c3d 7g7f')
    ).toEqual({
      depth: 8,
      multipv: 2,
      cp: -42,
    ...
    expect(
      parseUsiInfoLine('info depth 9 multipv 1 score mate -3 nodes 1200 pv 8b8h+')
    ).toMatchObject({ cp: -999_997, mate: -3, mateSign: -1, scoreKind: 'mate' });
    expect(
      parseUsiInfoLine('info depth 9 multipv 1 score mate 5 nodes 1200 pv 8b8h+')
    ).toMatchObject({ cp: 999_995, mate: 5, mateSign: 1 });
    expect(
      parseUsiInfoLine('info depth 9 multipv 1 score mate -0 nodes 1200 pv 8b8h+')
    ).toMatchObject({ cp: -1_000_000, mateSign: -1 });

    expect(
      parseUsiInfoLine('info depth 9 multipv 1 score cp 100 lowerbound nodes 1200 pv 7g7f')
    ).toBeNull();
    expect(
      parseUsiInfoLine('info depth 9 multipv 1 score cp 100 upperbound nodes 1200 pv 7g7f')
    ).toBeNull();
    expect(parseUsiInfoLine('info depth 9 score cp 100 pv 7g7f')).toBeNull();
    expect(
      parseUsiInfoLine('info depth 9 multipv nope score cp 100 nodes 1200 pv 7g7f')
    ).toBeNull();
  });
  ...
  it('can require the exact completed depth for fixed-depth searches', () => {
    const accumulator = new UsiMultiPvAccumulator({ multipv: 2, requiredDepth: 7 });
    accumulator.push([
  ...
  it('tombstones an older exact score when a newer bound update arrives', () => {
    const accumulator = new UsiMultiPvAccumulator({ multipv: 2, requiredDepth: 8 });
    accumulator.push([
      'info depth 8 multipv 1 score cp 20 nodes 200 pv 7g7f',
  ...
  it('accepts only a final exact mate when a forced fixed-depth search terminates early', () => {
    ...
    const noFallback = new UsiMultiPvAccumulator({ multipv: 1, requiredDepth: 18 });
    ...
    expect(() => unexpectedRank.finish()).toThrow(/incomplete MultiPV/);

    expect(() => new UsiMultiPvAccumulator({
      multipv: 2,
      requiredDepth: 18,
      allowTerminalMateBeforeRequiredDepth: true,
  ...
  it('rejects duplicate moves in a completed snapshot and missing bestmove', () => {
    ...
    const fixedDepthMismatch = new UsiMultiPvAccumulator({ multipv: 1, requiredDepth: 7 });
