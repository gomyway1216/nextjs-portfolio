...
describe("USI MultiPV parsing", () => {
  it("parses exact cp and mate PV records and excludes bounds", () => {
    expect(
      parseUsiInfoLine(
        "info depth 8 seldepth 12 multipv 2 score cp -42 nodes 901 pv 3c3d 7g7f",
      ),
    ).toEqual({
      depth: 8,
      multipv: 2,
    ...
    expect(
      parseUsiInfoLine(
        "info depth 9 multipv 1 score mate -3 nodes 1200 pv 8b8h+",
      ),
    ).toMatchObject({
      cp: -999_997,
      mate: -3,
    ...
    expect(
      parseUsiInfoLine(
        "info depth 9 multipv 1 score mate 5 nodes 1200 pv 8b8h+",
      ),
    ).toMatchObject({
      cp: 999_995,
      mate: 5,
    ...
    expect(
      parseUsiInfoLine(
        "info depth 9 multipv 1 score mate -0 nodes 1200 pv 8b8h+",
      ),
    ).toMatchObject({
      cp: -1_000_000,
      mateSign: -1,
    ...
    expect(
      parseUsiInfoLine(
        "info depth 9 multipv 1 score cp 100 lowerbound nodes 1200 pv 7g7f",
      ),
    ).toBeNull();
    expect(
      parseUsiInfoLine(
        "info depth 9 multipv 1 score cp 100 upperbound nodes 1200 pv 7g7f",
      ),
    ).toBeNull();
    expect(parseUsiInfoLine("info depth 9 score cp 100 pv 7g7f")).toBeNull();
    expect(
      parseUsiInfoLine(
        "info depth 9 multipv nope score cp 100 nodes 1200 pv 7g7f",
      ),
    ).toBeNull();
  });
  ...
  it("rejects unsafe integer tokens and non-canonical PV moves", () => {
    ...
    expect(
      parseUsiInfoLine(
        "info depth 9 multipv 1 score cp 100 nodes 1200 pv 7g7f ???",
      ),
    ).toBeNull();
    expect(
      parseUsiInfoLine(
        "info depth 9 multipv 1 score cp 100 nodes 1200 pv P*5e",
      ),
    ).toMatchObject({
      move: "P*5e",
      pv: ["P*5e"],
    ...
    expect(
      parseUsiInfoLine(
        "info depth 9 depth 10 multipv 1 score cp 100 nodes 1200 pv 7g7f",
      ),
    ).toBeNull();
    expect(
      parseUsiInfoLine(
        "info depth 9 multipv 1 score cp 100 nodes 1200 nodes 1300 pv 7g7f",
      ),
    ).toBeNull();
    expect(
      parseUsiInfoLine(
        "info depth 9 multipv 1 multipv 2 score cp 100 nodes 1200 pv 7g7f",
      ),
    ).toBeNull();
    expect(
      parseUsiInfoLine(
        "info depth 9 multipv 1 score cp 100 score cp 101 nodes 1200 pv 7g7f",
      ),
    ).toBeNull();
    expect(
      parseUsiInfoLine(
        "info depth 9 multipv 1 score cp 100 nodes 1200 pv 7g7f pv 2g2f",
      ),
    ).toBeNull();
  });
  ...
  it("rejects a historical required-depth snapshot after later rank updates", () => {
    ...
    expect(() => accumulator.finish()).toThrow(
      /did not end with exact updates/,
    );
  });
  ...
  it("fails closed after malformed structured evidence even when a later line is valid", () => {
    ...
    expect(() => accumulator.finish()).toThrow(
      /malformed structured teacher evidence/,
    );
  });
  ...
  it("accepts only a final exact mate when a forced fixed-depth search terminates early", () => {
    ...
    earlyCp.push(
      "info depth 16 multipv 1 score cp -40 nodes 966 pv 5h4h\nbestmove 5h4h",
    );
  ...
  it("rejects duplicate moves in a completed snapshot and missing bestmove", () => {
    ...
    fixedDepthMismatch.push(
      [
        "info depth 7 multipv 1 score cp 20 nodes 200 pv 7g7f",
        "bestmove 5g5f",
      ].join("\n"),
    );
    expect(() => fixedDepthMismatch.finish()).toThrow(
      /does not match completed PV1/,
    );
  });

  it("rejects malformed, terminal, or trailing-token bestmove values", () => {
    for (const bestmove of [
      "???",
      "resign",
      "win",
      "7g7f garbage",
      "7g7f ponder ???",
      "7g7f ponder 3c3d trailing",
    ]) {
      const accumulator = new UsiMultiPvAccumulator({
        multipv: 1,
        requiredDepth: 8,
      ...
      expect(() => accumulator.finish(), bestmove).toThrow(
        /malformed bestmove|terminal bestmove/,
      );
    }
  ...
  it("publishes the last exact snapshot below a dual node cap with an ordered witness", () => {
    const accumulator = new UsiMultiPvAccumulator({
      multipv: 1,
      requiredDepth: 18,
      nodeCap: 100_000_000,
    ...
    accumulator.push(
      [
        "info depth 15 multipv 1 score cp -3360 nodes 95168959 pv G*6a 5a4b",
        "info depth 16 multipv 1 score cp -3360 nodes 100000012 pv G*6a 5a4b",
        "bestmove G*6a",
      ].join("\n"),
    );
    expect(accumulator.finish()).toMatchObject({
      depth: 15,
      observedNodes: 95_168_959,
      lines: [{ cp: -3360, nodes: 95_168_959 }],
      dualBound: {
        terminationReason: "node-cap",
        requestedDepth: 18,
        nodeCap: 100_000_000,
        minimumCompletedDepth: 15,
        deepestCompleteExactDepth: 15,
        maximumObservedNodes: 100_000_012,
        maximumObservedDepth: 16,
        discardedAtOrAboveNodeCapUpdates: 1,
        capWitnessDepth: 16,
        capWitnessNodes: 100_000_012,
        selectedPrecedesWitness: true,
      },
    });
  });

  it("fails closed without a higher-depth, final, structured cap witness", () => {
    const run = (lines: readonly string[]) => {
      ...
      accumulator.push([...lines, "bestmove 7g7f"].join("\n"));
      return () => accumulator.finish();
    };
    expect(
      run([
        "info depth 15 multipv 1 score cp -40 nodes 95000000 pv 7g7f",
        "info depth 16 nodes 100000001",
      ]),
    ).toThrow(/incomplete MultiPV/);
    expect(
      run([
        "info depth 15 multipv 1 score cp -40 nodes 95000000 pv 7g7f",
        "info depth 15 multipv 1 score cp -39 nodes 100000001 pv 7g7f",
      ]),
    ).toThrow(/incomplete MultiPV/);
    expect(
      run([
        "info depth 15 multipv 1 score cp -40 nodes 95000000 pv 7g7f",
        "info depth 16 multipv 1 score cp -39 nodes 100000001 pv 7g7f",
        "info depth 14 multipv 1 score cp -45 nodes 100000002 pv 7g7f",
      ]),
    ).toThrow(/incomplete MultiPV/);
  });

  it("accepts a valid bound witness, and records an uncapped depth termination", () => {
    const capped = new UsiMultiPvAccumulator({
      multipv: 1,
      requiredDepth: 18,
      nodeCap: 100_000_000,
      minimumCompletedDepth: 15,
    ...
    capped.push(
      [
        "info depth 17 multipv 1 score cp -70 nodes 90000000 pv 7g7f",
        "info depth 18 multipv 1 score cp -60 lowerbound nodes 100000005 pv 7g7f",
        "bestmove 7g7f",
      ].join("\n"),
    );
    expect(capped.finish()).toMatchObject({
      depth: 17,
      dualBound: {
        terminationReason: "node-cap",
        capWitnessDepth: 18,
        observedLowerboundUpdates: 1,
      },
    });

    const completed = new UsiMultiPvAccumulator({
      multipv: 1,
      requiredDepth: 18,
      nodeCap: 100_000_000,
      minimumCompletedDepth: 15,
    ...
    completed.push(
      [
        "info depth 18 multipv 1 score cp 18 nodes 2000000 pv 7g7f",
        "bestmove 7g7f",
      ].join("\n"),
    );
    expect(completed.finish()).toMatchObject({
      depth: 18,
      dualBound: {
        terminationReason: "depth",
        capWitnessDepth: null,
        capWitnessNodes: null,
        selectedPrecedesWitness: false,
        completedIterationWitnessDepth: 18,
      },
    });
  });

  it("rejects invalid dual-bound accumulator shapes", () => {
    expect(
      () =>
        new UsiMultiPvAccumulator({
          multipv: 12,
    ...
    ).toThrow(/requires minimumCompletedDepth/);
    expect(
      () =>
        new UsiMultiPvAccumulator({
          multipv: 1,
    ...
    ).toThrow(/nodeCap requires requiredDepth/);
  });
});
...
describe("buildGo", () => {
  it("builds fixed-node and fixed-depth commands", () => {
    ...
    expect(buildGo({ depth: 8 }, ["7g7f", "2g2f"])).toBe(
      "go depth 8 searchmoves 7g7f 2g2f",
    );
    expect(
      buildGo({ depth: 18, nodes: 100_000_000, minimumCompletedDepth: 15 }, [
        "G*6a",
      ]),
    ).toBe("go depth 18 nodes 100000000 searchmoves G*6a");
  });

  it("requires positive integral limits and an explicit valid dual depth floor", () => {
    expect(() => buildGo({})).toThrow(/at least one/);
    expect(() => buildGo({ nodes: 100, depth: 8 })).toThrow(
      /requires minimumCompletedDepth/,
    );
    expect(() =>
      buildGo({ nodes: 100, depth: 8, minimumCompletedDepth: 9 }),
    ).toThrow(/must not exceed/);
    expect(() => buildGo({ nodes: 100, minimumCompletedDepth: 8 })).toThrow(
      /dual depth\/node/,
    );
    expect(() => buildGo({ nodes: 0 })).toThrow(/positive integer/);
    expect(() => buildGo({ depth: 1.5 })).toThrow(/positive integer/);
    expect(() => buildGo({ depth: Number.MAX_SAFE_INTEGER + 1 })).toThrow(
      /positive integer/,
    );
    expect(() => buildGo({ nodes: 10 }, ["7g 7f"])).toThrow(
      /invalid USI searchmove/,
    );
    expect(() => buildGo({ nodes: 10 }, ["???"])).toThrow(
      /invalid USI searchmove/,
    );
  });
});
