...
// Root-policy student boundary -------------------------------------------------
//
// Only a move-key permutation crosses into WASM. Student CP/residual values do
// not exist in this module and therefore cannot reach evaluation, alpha/beta,
// pruning or TT state. The pending receipt is dual-hash + sequence + count
// bound and becomes active only after it exactly matches the eager legal root
// universe generated by this production search.
const ROOT_POLICY_MAX_MOVES: i32 = MAX_MOVES;
const rootPolicyMoveKey = new StaticArray<i32>(ROOT_POLICY_MAX_MOVES);
const rootPolicyRank = new StaticArray<i32>(ROOT_POLICY_MAX_MOVES);
const rootPolicyEntrySet = new StaticArray<bool>(ROOT_POLICY_MAX_MOVES);
let rootPolicyArmed: bool = false;
let rootPolicyActive: bool = false;
let rootPolicySequence: i32 = 0;
let rootPolicySearchSequence: i32 = 0;
let rootPolicyPositionHashA: i32 = 0;
let rootPolicyPositionHashB: i32 = 0;
let rootPolicyMoveCount: i32 = 0;
let rootPolicyInstallFault: i32 = 0;

// Diagnostics are intentionally separate from the live receipt state so the
// mandatory end-of-search clear does not erase admission evidence.
let lastRootPolicyAccepted: bool = false;
let lastRootPolicyApplyCount: i32 = 0;
let lastRootPolicyNonRootApplyCount: i32 = 0;
let lastRootPolicyFault: i32 = 0;

function clearRootPolicyRankStateAS(resetInstallFault: bool): void {
  rootPolicyArmed = false;
  rootPolicyActive = false;
  rootPolicySequence = 0;
  rootPolicySearchSequence = 0;
  rootPolicyPositionHashA = 0;
  rootPolicyPositionHashB = 0;
  rootPolicyMoveCount = 0;
  if (resetInstallFault) rootPolicyInstallFault = 0;
}

/** Clear pending/active rank state. Last-search diagnostics remain readable. */
export function clearRootPolicyRank(): void {
  clearRootPolicyRankStateAS(true);
}

/**
 * Start an atomic rank install against the already-finalized root position.
 * Returns 1 on acceptance, 0 on fail-closed rejection.
 */
export function beginRootPolicyRank(
  sequence: i32,
  positionHashA: i32,
  positionHashB: i32,
  moveCount: i32,
  ...
  if (
    sequence <= 0 ||
    moveCount <= 0 ||
    moveCount > ROOT_POLICY_MAX_MOVES ||
    positionHashA != getHashVal() ||
    positionHashB != getSecondaryHashVal()
  ) {
    rootPolicyInstallFault = 1;
    return 0;
  }
  rootPolicySequence = sequence;
  rootPolicyPositionHashA = positionHashA;
  rootPolicyPositionHashB = positionHashB;
  rootPolicyMoveCount = moveCount;
  for (let i = 0; i < moveCount; i++) unchecked(rootPolicyEntrySet[i] = false);
  return 1;
}

/** Add one unique move-key/rank pair to the pending receipt. */
export function setRootPolicyRankEntry(index: i32, moveKey: i32, rank: i32): i32 {
  if (
    rootPolicyInstallFault != 0 ||
    rootPolicySequence <= 0 ||
    index < 0 ||
    index >= rootPolicyMoveCount ||
    moveKey == 0 ||
    rank < 0 ||
    rank >= rootPolicyMoveCount ||
    unchecked(rootPolicyEntrySet[index])
  ) {
    rootPolicyInstallFault = 1;
    rootPolicyArmed = false;
    return 0;
  }
  for (let i = 0; i < rootPolicyMoveCount; i++) {
    if (!unchecked(rootPolicyEntrySet[i])) continue;
    if (
      unchecked(rootPolicyMoveKey[i]) == moveKey ||
      unchecked(rootPolicyRank[i]) == rank
    ) {
      rootPolicyInstallFault = 1;
      rootPolicyArmed = false;
      return 0;
  ...
  unchecked(rootPolicyMoveKey[index] = moveKey);
  unchecked(rootPolicyRank[index] = rank);
  unchecked(rootPolicyEntrySet[index] = true);
  return 1;
}

/** Complete the atomic receipt after all entries were written exactly once. */
export function commitRootPolicyRank(sequence: i32): i32 {
  if (
    rootPolicyInstallFault != 0 ||
    sequence <= 0 ||
    sequence != rootPolicySequence
  ) {
    rootPolicyInstallFault = 1;
    rootPolicyArmed = false;
    return 0;
  ...
  for (let i = 0; i < rootPolicyMoveCount; i++) {
    if (!unchecked(rootPolicyEntrySet[i])) {
      rootPolicyInstallFault = 1;
      rootPolicyArmed = false;
      return 0;
    }
  }
  rootPolicyArmed = true;
  return 1;
}

/** Bind the next search to the same positive sequence as the receipt. */
export function setRootPolicySearchSequence(sequence: i32): void {
  rootPolicySearchSequence = sequence > 0 ? sequence : 0;
}

function rootPolicyRankForKeyAS(key: i32): i32 {
  for (let i = 0; i < rootPolicyMoveCount; i++) {
    if (unchecked(rootPolicyMoveKey[i]) == key) {
      return unchecked(rootPolicyRank[i]);
    }
  }
  return -1;
}
...
/**
 * Validate the pending rank against the exact eager legal root universe.
 * Any duplicate/missing/extra/stale state discards the complete rank.
 */
function activateRootPolicyRankAS(rootN: i32): void {
  lastRootPolicyAccepted = false;
  lastRootPolicyApplyCount = 0;
  lastRootPolicyFault = rootPolicyInstallFault;

  if (!rootPolicyArmed && rootPolicySearchSequence == 0) return; // disabled parity
  if (
    !rootPolicyArmed ||
    rootPolicyInstallFault != 0 ||
    rootPolicySearchSequence <= 0 ||
    rootPolicySequence != rootPolicySearchSequence ||
    rootPolicyPositionHashA != getHashVal() ||
    rootPolicyPositionHashB != getSecondaryHashVal()
  ) {
    lastRootPolicyFault = 2; // malformed/stale receipt
    clearRootPolicyRankStateAS(false);
    return;
  }
  if (rootN != rootPolicyMoveCount) {
    lastRootPolicyFault = 3; // production legal-universe count mismatch
    clearRootPolicyRankStateAS(false);
    return;
  }
  for (let i = 0; i < rootN; i++) {
    const key = jsMoveKeyOf(unchecked(moveBuf[i]));
    let matches = 0;
    for (let j = 0; j < rootPolicyMoveCount; j++) {
      if (unchecked(rootPolicyMoveKey[j]) == key) matches++;
    }
    if (matches != 1) {
      lastRootPolicyFault = 3;
      clearRootPolicyRankStateAS(false);
      return;
    }
  }
  rootPolicyActive = true;
  lastRootPolicyAccepted = true;
}

/**
 * Stable root-only reorder after the ordinary engine score sort.
 * Ranked legal moves precede unranked pseudo-illegal moves; each class keeps
 * its prescribed/stable relative order. Lower integer rank is better.
 */
function applyRootPolicyRankAS(base: i32, n: i32, ply: i32): void {
  if (ply != 0) {
    if (rootPolicyActive) lastRootPolicyNonRootApplyCount++;
    return;
  }
  if (!rootPolicyActive) return;
  if (
    rootPolicyPositionHashA != getHashVal() ||
    rootPolicyPositionHashB != getSecondaryHashVal()
  ) {
    lastRootPolicyAccepted = false;
    lastRootPolicyFault = 2;
    clearRootPolicyRankStateAS(false);
    return;
  }
  for (let i = 1; i < n; i++) {
    const move = unchecked(moveBuf[base + i]);
    const score = unchecked(moveScoreBuf[base + i]);
    const rank = rootPolicyRankForKeyAS(jsMoveKeyOf(move));
    if (rank < 0) continue;

    let j = i - 1;
    while (j >= 0) {
      const previous = unchecked(moveBuf[base + j]);
      const previousRank = rootPolicyRankForKeyAS(jsMoveKeyOf(previous));
      if (previousRank >= 0 && previousRank < rank) break;
      unchecked(moveBuf[base + j + 1] = previous);
      unchecked(moveScoreBuf[base + j + 1] = moveScoreBuf[base + j]);
      j--;
    }
    unchecked(moveBuf[base + j + 1] = move);
    unchecked(moveScoreBuf[base + j + 1] = score);
  }
  lastRootPolicyApplyCount++;
}
...
function searchNodeAS(depthLeft: i32, alpha: i32, beta: i32, ply: i32): i32 {
  ...
  if (ply == 0) applyRootPolicyRankAS(base, n, ply);
  orderDepthLeft = 99;
...
export function searchBestMove(maxTimeMs: f64, maxDepth: i32, quiescenceDepthMax: i32): i32 {
  ...
  lastRootPolicyAccepted = false;
  lastRootPolicyApplyCount = 0;
  lastRootPolicyNonRootApplyCount = 0;
  lastRootPolicyFault = rootPolicyInstallFault;
  stopped = false;
  nodeCount = 0;
  leafCount = 0;
  ...
  // Only the exact eager legal universe can activate a pending rank. The
  // iterative search subsequently generates pseudo-legal root moves; its
  // unranked illegal candidates stay behind ranked legal moves in stable
  // ordinary-score order.
  activateRootPolicyRankAS(rootN);
  ...
  applyRootPolicyRankAS(0, rootN, 0);
...
export function getLastRootPolicyRankAccepted(): i32 {
  return lastRootPolicyAccepted ? 1 : 0;
}

export function getLastRootPolicyRankApplyCount(): i32 {
  return lastRootPolicyApplyCount;
}

export function getLastRootPolicyRankNonRootApplyCount(): i32 {
  return lastRootPolicyNonRootApplyCount;
}

export function getLastRootPolicyRankFault(): i32 {
  return lastRootPolicyFault;
}
