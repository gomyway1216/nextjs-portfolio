...
if (!weightsPath || weightsPath.startsWith("--")) {
  console.error(
    "usage: node -r tsx/cjs wasm-spike/match-nnue-vs-v3.ts <weights.bin> [--vs otherWeights.bin] [--games 16] [--ms 200] [--seed 1] [--k 600] [--scale-numer 1] [--scale-denom 1] [--max-plies 256] [--wasm-path candidate.wasm] [--wasm-path-b production.wasm] [--lazy-picker-a-min-moves 64] [--lazy-picker-b-min-moves 64]",
  );
  process.exit(2);
}
...
const EXPECTED_WASM_SHA_B =
  argStr("--wasm-sha-b") ??
  (WASM_PATH_B === WASM_PATH ? EXPECTED_WASM_SHA : null);
...
function setupNnueInstance(
  ...
  if (
    ...
    if (weightsBin.byteLength !== wasm.getDpaHalfkp64Rki16WeightsSize()) {
      throw new Error(`${label}: RKI16 payload size differs from candidate runtime`);
    }
    // The lazy candidate allocation grows memory. Resolve the pointer first,
    // then read the fresh buffer so a detached pre-grow ArrayBuffer is never
    // captured by the Uint8Array constructor.
    const candidateWeightsPtr = wasm.getDpaHalfkp64Rki16WeightsPtr();
    new Uint8Array(
      wasm.memory.buffer,
      candidateWeightsPtr,
    ...
    if (wasm.setDpaHalfkp64Rki16RuntimeEnabled(1) !== 1) {
      throw new Error(`${label}: failed to enable RKI16 runtime`);
    }
    return 0;
  }
  const buckets = bucketOverride > 0
      ? bucketOverride
...
function main(): void {
  ...
  if (
    WASM_PATH_B !== undefined &&
    EXPECTED_WASM_SHA_B !== null &&
    createHash("sha256").update(readFileSync(WASM_PATH_B)).digest("hex") !==
      EXPECTED_WASM_SHA_B
  ) {
    throw new Error(
      "opponent WASM SHA-256 differs from the preregistered asset",
    );
  }
  ...
  configureResearchLazyMovePicker(
    wasmA,
    "A",
    LAZY_PICKER_A_MIN_MOVES,
    WASM_PATH,
  );

  // Instance B: second NNUE (--vs) or the stock hand-crafted evaluateV3Full.
  const wasmB = loadShogiWasm(WASM_PATH_B) as ShogiNnueSearchWasm;
  let opponentName = "V3";
  if (weightsPathB) {
  ...
  console.log(
    `=== match: WASM+NNUE-A(${weightsPath}, runtime=HalfKP64-RKI16, K=${SCALE_K_A}, outScale=${SCALE_NUMER}/${SCALE_DENOM}) ` +
      ...
      `runtime=A:${WASM_PATH ?? "production"},B:${WASM_PATH_B ?? "production"}, fixed-time-ms=${MOVE_MS}, ` +
      `max-plies=${MAX_PLIES}, ` +
